...
| **UI-2** (no hardcoded colour) | **369** lint warnings (re-measured 2026-08-29, down from 1,906 → 1,741 → 1,447 → 1,330 → 858 → 369 across this session's own sweep passes) | Ratcheted debt. Rule enforced for new code; existing swept incrementally. Ratchet may only decrease. **As of 2026-08-29 this is also UI-8's blocker** — every one of these warnings is a spot that will render wrong (wrong background, unreadable text, a light-tuned shadow/gradient) in dark mode, not just an off-brand colour in light mode. Phased sweep plan (measured per-directory counts as of 2026-08-29, worst-first within each phase): **Phase 1 — shared, all roles — DONE** (`components/`, `layouts/` remainder). **Phase 2 — patient/guest, mobile-first tier — DONE** (`auth/`, `public/`, `patient/`, `onboarding/`, `booking/`) — highest product priority per §5's tiering table; live-verified in dark mode on the login page and the public landing page (search card, filters sidebar, doctor-result cards). **Phase 3 — clinician, tablet-first tier — DONE** (`calendar/index.jsx`, `appointments/{detail,index,edit}.jsx`, `clinician/{Calendar,Dashboard,Patients,Availability,EncounterWorkspace}.jsx`, 472 warnings fixed across 9 files) — see the `clinician-dashboard`/`frontend-platform` bundle for the two real, non-colour bugs this pass also found and fixed (a lunch-break day-of-week convention mismatch, an unreachable hover-popover link) and the separate `src/index.css` global-CSS finding (below). **Phase 4 — staff/manager/admin, desktop-dense tier — IN PROGRESS**, ~290 fixed so far this session: `finances/index.jsx`, `messages/index.jsx`, `admin/users/index.jsx`, `staff/{edit,index,new}.jsx`, `reviews/index.jsx`, `manager/Dashboard.jsx`, `analytics/index.jsx` all fully clean (0 warnings each); `layouts/AppShell.jsx` and `settings/index.jsx` reduced to documented-exception-only residue (see the two new §22 rows below — fixed sidebar/nav chrome, user-selectable colour pickers). Two real hard-rule additions came out of this pass, both under **UI-8** above: (a) a hex→token conversion must check the colour was ever brand-consistent, not just nearest-token-match (the finances "Export Report" button/Cash Drawer tab shipped an off-brand blue this way — now converted to `primary`); (b) `MuiAlert`'s own dark-mode default background can render darker than its container card, which reads as a missing-spacing bug but is actually a contrast bug — fixed via a new `theme.components.MuiAlert` override, not by touching any margin. Remaining Phase 4 files, worst-first (re-measure before trusting): `manager/clinics/index.jsx` (22), `patients/detail.jsx` (19), `patients/index.jsx` (18), `waiting-room/index.jsx` (16), `admin/Organizations.jsx` (14), `test-results/index.jsx` (13), `dashboard/index.jsx` (12), `notifications/index.jsx` (12), `manager/services/detail.jsx` (10), `clinicians/{Create,Edit}ClinicianPage.jsx` (9 each), `patients/CreatePatientPage.jsx` (9), `components/Clinicians/{ClinicianCard,ClinicianProfileDrawer}.jsx` (8 each), `components/Dashboard/{RecentAppointmentsTable,ServicePieChart}.jsx` (8 each), `admin/Communications.jsx` (8), `clinicians/index.jsx` (8), `layouts/PublicLayout.jsx` (7 — likely the documented footer exception, verify), `manager/rooms/detail.jsx` (7), `manager/services/index.jsx` (7), `patients/EditPatientPage.jsx` (7), `video/index.jsx` (7), `components/ConfettiExplosion.jsx` (6 — likely a decorative multi-hue exception, verify), plus ~15 smaller files at 1–5 each. A future session resuming this should re-measure with `npx eslint src/pages src/components src/layouts` before trusting these counts. Recurring patterns worth knowing before resuming: (1) several files hand-picked their own per-status/per-role/per-video hex map (STATUS_CONFIG-shaped) — replace these with the shared `theme.palette.appointmentStatus` extension or a small `useTheme()`-driven tone helper, not a fresh hex map; (2) check for a `BRAND`/`TEAL`/`STITCH_BRAND`-style module-level colour constant used in template-literal alpha tricks (`` `${BRAND}08` ``) before deleting it — grep every usage first, several files had more call sites than an initial hex-literal grep alone revealed; (3) a component consuming `theme.palette.appointmentStatus` needs a real `<ThemeProvider theme={createAppTheme(...)}>` in its test, not a bare render with no theme at all — see the new UI-8 entry above; (4) `.jsx`-only lint can never catch a hardcoded colour sitting in a global `.css` file — see the `src/index.css` finding immediately below, found only because a live user asked "check table hover" during this exact phase; (5) a widely-referenced module-level brand constant (`TEAL`, `BRAND`) used in dozens of template-literal/prop call sites is safest fixed by redeclaring it *inside* the component as `theme.palette.primary.main`, not by touching every call site; (6) four now-established deliberate-exception categories exist and should be documented inline, not left as unexplained lint noise, when found again: marketing/brand panels (fixed gradient independent of theme), terminal/code-viewer boxes (fixed dark box), fixed sidebar/nav-rail chrome (`AppShell.jsx`'s `TEAL`/`#1A2332` — the sidebar deliberately stays one exact colour across both light/dark app modes), and user-selectable colour pickers (`settings/index.jsx`'s `ACCENT_COLORS`, org-branding `primaryColor`/`secondaryColor` — the picked hex *is* the feature). |
