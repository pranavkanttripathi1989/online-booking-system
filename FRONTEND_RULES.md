...
| **UI-2** (no hardcoded colour) | **1,330** lint warnings (re-measured 2026-08-29, down from 1,906 → 1,741 → 1,447 → 1,330 across this session's own three sweep passes) | Ratcheted debt. Rule enforced for new code; existing swept incrementally. Ratchet may only decrease. **As of 2026-08-29 this is also UI-8's blocker** — every one of these warnings is a spot that will render wrong (wrong background, unreadable text, a light-tuned shadow/gradient) in dark mode, not just an off-brand colour in light mode. Phased sweep plan (measured per-directory counts as of 2026-08-29, worst-first within each phase): **Phase 1 — shared, all roles — DONE** (`components/`, `layouts/` remainder). **Phase 2 — patient/guest, mobile-first tier — DONE** (`auth/`, `public/`, `patient/`, `onboarding/`, `booking/`) — highest product priority per §5's tiering table; live-verified in dark mode on the login page and the public landing page (search card, filters sidebar, doctor-result cards). **Phase 3 — clinician, tablet-first tier — not started** (`clinician/`, `calendar/`, `appointments/`, ~420). **Phase 4 — staff/manager/admin, desktop-dense tier — not started** (`staff/`, `finances/`, `manager/`, `admin/`, `patients/`, `analytics/`, `clinicians/`, `messages/`, `reviews/`, `settings/` remainder, ~660) — largest volume, lowest per-screen risk. A future session resuming this should re-measure with `npx eslint src/pages src/components src/layouts` before trusting these counts, then pick up Phase 3 next. A recurring pattern worth knowing before resuming: several files hand-picked their own per-status/per-role/per-video hex map (STATUS_CONFIG-shaped) — replace these with the shared `theme.palette.appointmentStatus` extension or a small `useTheme()`-driven tone helper, not a fresh hex map; and check for a `BRAND`/`TEAL`-style module-level colour constant used in template-literal alpha tricks (`` `${BRAND}08` ``) before deleting it — grep every usage first, several of Phase 2's files had more call sites than an initial hex-literal grep alone revealed |
