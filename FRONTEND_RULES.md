...
| **FORM-20** (rich text editor for reader-facing free text) | `src/components/shared/RichTextEditor.jsx` (TipTap v3, lazy-loaded) now exists and is wired into `EncounterWorkspace.jsx`'s 8 clinical note sections (2026-08-29) — the first real instance closed. ~57 other pre-existing plain `multiline` fields remain (prescription instructions, messages, bios, policy/template bodies, review text) | Applies to new fields from today by reusing the real component; the remaining fields retrofit incrementally, own future slice per field/page |
