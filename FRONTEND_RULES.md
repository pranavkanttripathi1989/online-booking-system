...
| **UI-2** (no hardcoded colour) | **1,741** lint warnings (re-measured 2026-08-29, down from 1,906) | Ratcheted debt. Rule enforced for new code; existing swept incrementally. Ratchet may only decrease. **As of 2026-08-29 this is also UI-8's blocker** — every one of these warnings is a spot that will render wrong (wrong background, unreadable text, a light-tuned shadow/gradient) in dark mode, not just an off-brand colour in light mode. Phased sweep plan (measured per-directory counts as of 2026-08-29, worst-first within each phase): **Phase 1 — shared, all roles** (`components/` ~277, `layouts/` remainder ~107) — done first because it improves every role at once. **Phase 2 — patient/guest, mobile-first tier** (`auth/`, `public/`, `patient/`, `onboarding/`, `booking/`, ~150) — highest product priority per §5's tiering table. **Phase 3 — clinician, tablet-first tier** (`clinician/`, `calendar/`, `appointments/`, ~420). **Phase 4 — staff/manager/admin, desktop-dense tier** (`staff/`, `finances/`, `manager/`, `admin/`, `patients/`, `analytics/`, `clinicians/`, `messages/`, `reviews/`, `settings/` remainder, ~660) — largest volume, lowest per-screen risk. A future session resuming this should re-measure with `npx eslint src/pages src/components src/layouts` before trusting these counts, then pick up the next incomplete phase in order |
