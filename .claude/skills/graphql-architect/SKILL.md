...
license: MIT
metadata:
  upstream_risk_tag: critical
  source: https://github.com/sickn33/antigravity-awesome-skills/blob/main/skills/graphql-architect/SKILL.md
  source_repo: sickn33/antigravity-awesome-skills
  date_added: '2026-02-27'
  vetted: >-
    Reviewed 2026-08-18 before vendoring: pure static Markdown reference
    content (capability checklists, no executable instructions), no external
    calls, no prompt-injection patterns. Source repo verified via the GitHub
    API directly (MIT-licensed, 45k+ stars). The upstream risk:critical tag
    reflects the aggregator's own categorization for anything touching
    security/auth/production topics, not a finding of malicious content —
    kept here (as upstream_risk_tag) rather than dropped, since it is a
    legitimate signal to review any security/authz guidance this skill
    produces before applying it, matching the skill's own "Limitations"
    section below. This backend is NestJS + Apollo GraphQL code-first
    (schema built from TypeScript decorators, not SDL) — the
    federation/gateway and schema-registry sections are aspirational for a
    single-service backend at this project's current scale; the
    DataLoader/N+1, query-complexity, field-level-authorization, and
    subscription-scaling sections apply directly.
---
