...
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/main/skills/react-patterns/SKILL.md
  license: MIT
  vetted: >-
    Reviewed 2026-08-18 before vendoring: pure static Markdown/code-sample
    content, no executable instructions, no external calls, no
    prompt-injection patterns. Source repo (affaan-m/ECC, MIT-licensed, 240k+
    stars, verified via the GitHub API directly) — same publisher already
    vetted for nestjs-patterns. Note: this project is a Vite SPA, not
    Next.js — the Server/Client Component (RSC) and Server Actions sections
    here don't apply directly; the hooks discipline, state-location decision
    tree, Suspense/error-boundary, forms (React Hook Form for complex
    cases), accessibility, and performance/memoization sections do.
---
...
Highlights (the source repo's fuller `rules/react/hooks.md` ruleset was not vendored, only this file):
...
Sibling skills referenced in the source repo (`react-performance`, `react-testing`, `frontend-patterns`, `accessibility`, `angular-developer`) and its `react-reviewer`/`react-build-resolver` agents and `/react-review`/`/react-build`/`/react-test` commands were **not** vendored into this project — only this one file was pulled in. If deeper coverage of any of those areas is needed later, fetch them the same way (from `affaan-m/ECC`, reviewed before adding).
