...
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/main/skills/error-handling/SKILL.md
  license: MIT
  vetted: >-
    Reviewed 2026-08-18, part of the same 8-file batch review as
    security-review (see that file for the full batch note). This project
    is TypeScript-only (NestJS backend, React frontend) — the Python/Go
    sections don't apply. Directly relevant to this backend's existing
    error-formatting rule (backend-hard-rules.md Rule 4: strip internals in
    production, never leak raw Prisma errors).
---
