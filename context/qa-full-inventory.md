...
## 7. Live Chrome MCP verification pass — 2026-08-19 (Priority 1 e2e-readiness check, newly-tested-11 domains)

Drove the real running stack as `manager@medibook.dev` and `admin@medibook.dev` to confirm which of this session's newly-unit-tested domains (`clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `products`, `reviews`, `rooms`, `services`, `users`) actually have their frontend page wired to the real backend — a prerequisite for writing real (non-mock) e2e specs against them per Priority 1's DoD.

**Confirmed real, live-verified against the actual backend (safe to write e2e specs against):**
- `manager/clinics.jsx` — real seeded clinics (MG Road Clinic, Koramangala Health Center, Admin Test Clinic).
- `admin/EmailTemplates.jsx` — real; did a full edit round-trip (changed the Appointment Cancellation subject, confirmed it persisted, reverted it) via the real `updateEmailTemplate` mutation.
- `admin/Languages.jsx` — real (English default=false / Hindi default=true, matching seed).
- `admin/ClinicianTypes.jsx` and `admin/RoomTypes.jsx` (`lookups` domain) — both real.
- `manager/products/index.jsx` — real; SKU format (`gp-consultation-mswwj5ii`) matches `ProductsService.generateSku()`'s timestamp-suffix pattern exactly.
- `manager/rooms/index.jsx` — real; paginated, real clinic names.
- `admin/users/index.jsx` Users Directory tab — real, all 8 real accounts (5 seed + 3 test accounts from earlier sessions).
- `admin/users/index.jsx` Permissions Matrix tab — real (`GetRBACData` query hits the actual `getPermissions`/`getRolePermissions` resolvers; response is genuinely `{getPermissions: [], getRolePermissions: []}` since no `Permissions` rows are seeded, matching the page's own "0 Permissions Defined" stat). **Minor UI bug, not fixed:** the matrix renders every resource's READ checkbox as checked regardless of this empty response — a rendering default, not a saved-state reflection; low risk (display-only unless someone clicks Save Changes without noticing), logged here rather than fixed since it's outside this pass's scope.
- `messages/index.jsx` — real, reconfirmed (still shows a prior session's "Chrome MCP live verification" test message).
- `reviews/index.jsx` — real; genuinely empty state ("0 total", "No reviews found") since no `createReview` path exists in the backend yet (matches `reviews.service.spec.ts`'s own note that no create path exists).

**Confirmed BROKEN — real backend exists and is correctly unit-tested, but the frontend page never actually calls it:**
- **`manager/services/index.jsx`'s inline `GetCatalogData` query is a genuine contract-mismatch bug**, not a "not yet wired" gap. It queries `getProductCategories`/`getProducts(clinicId, categoryId)` against the *public* dialect's `PublicProduct`/`PublicCancellationRule` types, but also requests fields (`sku`, `is_active`, `ruleType`, `feeType`, `feeAmount`, `hoursBeforeAppointment`) that don't exist on those public types at all — the query doesn't match either of the two real dialects (Architecture section) and 400s on every load with 10 separate `GRAPHQL_VALIDATION_FAILED` errors. The page's own error handling correctly catches this and shows a visible "Demo mode — Showing sample data. Backend is offline or unreachable." banner (so this isn't a *silent* mock fallback, Hard Rule 8's main concern), but the underlying `services` backend module (`backend/src/services/`, fully unit-tested this session: `services`, `service`, `createService`, `updateService`) is never actually reached. Root-caused via the chrome MCP network inspector (`GetCatalogData` → HTTP 400, full validation-error list captured); not fixed this pass — user explicitly deferred it (709-line page, real standalone rewiring task) in favor of continuing e2e coverage on the domains that do work. **No e2e spec exists for `services` as a result** — writing one now would only exercise mock data, contradicting the DoD's "against the real backend, not mocks" requirement.

**Already known, reconfirmed unchanged (no new finding):** `components/shared/NotificationBell.jsx` still on mock/stale demo data (already listed in §4's GAP list); `admin/Roles.jsx` still 100% `mocks/store.js`-driven, confirmed by reading the component source directly (`useMockData`/`useMockMutation`/`MockStore.createRole` etc., zero `gql`/gql import) — also already listed in §4.

### `public` domain — two real, separate bugs found chasing an e2e spec for the patient booking flow (2026-08-20)

Attempted to write an e2e spec for `public` (the last domain besides `services`/`staff` without one). Found two distinct, real bugs rather than a simple "not yet wired" gap:

1. **`getClinicianAvailability` has no `@Public()` and no `@Auth()`, so the global `GqlAuthGuard` 401s it for every logged-out caller.** It lives in `backend/src/availability/availability.resolver.ts`, tagged `// clinician/Availability.jsx self-service surface` — it was built for the logged-in clinician's own schedule editor. But `pages/public/doctor-profile.jsx`'s `GetClinicianProfile` query calls it in the *same combined query document* as `getClinician(id)` (which *is* `@Public()`), so — same failure mode as the already-fixed `manager/Dashboard.jsx` combined-query bug (§5 finding #2) — the one unauthorized field nulls out the entire response's `data`, and the whole doctor profile page shows a bare "Unauthorized" alert for any anonymous visitor. Reproduced live: `curl`'d a real clinician id (`getClinicians` → Sarah Mitchell, `8e9ed6bf-daf0-49cb-84f3-82c8c4ba80e7`), navigated a fresh unauthenticated browser context to `/doctor/8e9ed6bf-...`, captured the exact GraphQL response (`{"errors":[{"message":"Unauthorized",...,"path":["getClinicianAvailability"]}],"data":null}`).
2. **`/appointments/book` (the booking wizard, `pages/booking/index.jsx`) is itself gated behind login in `App.jsx`'s route tree**, even though its own GraphQL contract (`bookPatientAppointment`/`BookPatientAppointmentInput`, camelCase fields) was clearly built for the public/patient-self-serve dialect per this file's own Architecture section. Navigating there unauthenticated redirects straight to `/login` — reproduced live in the same fresh session.

Both are concrete, understood bugs (not ambiguous), but fixing either touches auth-guard annotations or routing structure — not something to change without confirming scope, matching how the `manager/services` contract-mismatch bug was handled (logged, deferred, not silently patched). **No e2e spec written for `public`** as a result — the flow doesn't currently work for an anonymous visitor at all, so a spec would either have to log in first (defeating the point of testing the *public* flow) or assert on broken behavior.

**Also confirmed still 100% mock, no new finding:** `pages/public/landing.jsx` — its clinician search still runs against a hardcoded `MOCK_DOCTORS` array behind a `setTimeout` (comment: `// ── Simulate GraphQL getClinicians ──`), matching §4's existing GAP listing.
