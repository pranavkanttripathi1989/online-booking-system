...
const END_IMPERSONATION_MUTATION = gql`
  mutation EndImpersonationFromShell {
    endImpersonation {
      success
      userErrors {
        message
...
const SEARCH_DATA = [
  ...
  { type: 'patient', label: 'Carol Davis', sub: 'Follow-Up · carol@example.com', path: '/patients', Icon: PersonRoundedIcon },
  {
    ...
    sub: 'General Practitioner · Meridian Central',
    path: '/clinicians',
...
const NAV_CONFIG = [
  ...
  { label: 'My Calendar', path: '/clinician/calendar', icon: <CalendarMonthIcon />, roles: ['clinician'] },
  {
    ...
    roles: ['admin', 'super_admin', 'manager', 'receptionist', 'staff'],
  },
  // ── Patients ──────────────────────────────────────────────────────────────
...
function SearchDropdown({ query, onSelect, activeIdx, setActiveIdx }) {
  ...
  const results =
    query.length >= 1
      ? SEARCH_DATA.filter(
          (d) => d.label.toLowerCase().includes(query.toLowerCase()) || d.sub.toLowerCase().includes(query.toLowerCase()),
        ).slice(0, 8)
    ...
    <Box sx={{ width: DRAWER_WIDTH, height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#1A2332', overflow: 'hidden' }}>
      ...
      >
        {branding?.logo_url ? (
          <Box
            component="img"
            src={resolveLogoSrc(branding.logo_url)}
      ...
      <Box sx={{ px: 2, pt: 2, pb: 1 }}>
        <Box sx={{ bgcolor: 'rgba(255,255,255,0.06)', borderRadius: 2, p: 1.5, display: 'flex', alignItems: 'center', gap: 1.25 }}>
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            <Typography variant="body2" fontWeight={700} noWrap sx={{ color: '#fff', fontSize: '0.83rem' }}>
              {displayName}
            </Typography>
            <Chip
              label={roleCfg.label}
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', py: 0.5, px: 1 }}>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <ListItemText
                primary={item.label}
                primaryTypographyProps={{ fontSize: '0.865rem', fontWeight: active ? 700 : 500, color: 'inherit' }}
              />
              {item.badge > 0 && (
                <Box
                  sx={{ bgcolor: '#D93025', color: '#fff', borderRadius: '10px', px: 0.8, py: 0.1, fontSize: '0.6rem', fontWeight: 700 }}
                >
                  {item.badge}
                </Box>
              )}
        ...
        {isManager && (
          <>
            <ListItemButton
              onClick={() => setExpandedManager((v) => !v)}
              sx={{
                borderRadius: '10px',
                py: 1.1,
                px: 1.5,
            ...
            >
              <ListItemIcon sx={{ minWidth: 36, color: 'rgba(255,255,255,0.38)', '& .MuiSvgIcon-root': { fontSize: '1.15rem' } }}>
                <SettingsIcon />
              </ListItemIcon>
              <ListItemText primary="Manager" primaryTypographyProps={{ fontSize: '0.865rem', fontWeight: 500, color: 'inherit' }} />
              {expandedManager ? (
                <ExpandLessIcon sx={{ fontSize: '1rem', color: 'rgba(255,255,255,0.40)' }} />
              ) : (
                <ExpandMoreIcon sx={{ fontSize: '1rem', color: 'rgba(255,255,255,0.40)' }} />
              )}
            </ListItemButton>
                  ...
                  return (
                    <ListItemButton
                      ...
                      sx={{
                        ...
                        px: 1.5,
                        mb: 0.3,
                        color: act ? '#fff' : 'rgba(255,255,255,0.48)',
                        background: act ? `${TEAL}CC` : 'transparent',
                        '&:hover': { bgcolor: 'rgba(255,255,255,0.06)', color: '#fff' },
                        '&.Mui-selected': { background: `${TEAL}CC`, color: '#fff' },
                      }}
                    >
                      <ListItemIcon sx={{ minWidth: 32, color: 'inherit', '& .MuiSvgIcon-root': { fontSize: '1rem' } }}>
                        {child.icon}
                      </ListItemIcon>
                      <ListItemText
                        primary={child.label}
                        primaryTypographyProps={{ fontSize: '0.82rem', fontWeight: act ? 700 : 400, color: 'inherit' }}
                      />
                    </ListItemButton>
                  )
                })}
      ...
      <Box sx={{ px: 2, py: 1.5 }}>
        <Box
          ...
          sx={{
            ...
            fontWeight: 700,
            fontSize: '0.78rem',
            '&:hover': { bgcolor: 'rgba(217,48,37,0.26)' },
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mr: 3, flexShrink: 0 }}>
          {branding?.logo_url ? (
            <Box
              ...
              src={resolveLogoSrc(branding.logo_url)}
              alt={`${branding.name} logo`}
              sx={{ width: 32, height: 32, borderRadius: 1.5, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.18)' }}
            />
          ) : (
            <Box
              sx={{
          ...
          {branding?.logo_url ? (
            <Typography
              ...
              fontWeight={800}
              noWrap
              sx={{ color: '#fff', letterSpacing: '-0.5px', lineHeight: 1, maxWidth: 160 }}
            >
              {branding.name}
            </Typography>
          ) : (
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.25, flex: 1 }}>
          {mainItems.map((item) => {
            ...
            return (
              <Button
                key={item.path}
                startIcon={<Box sx={{ '& .MuiSvgIcon-root': { fontSize: '0.9rem' }, display: 'flex' }}>{item.icon}</Box>}
...
export default function AppShell() {
  ...
  const handleExitImpersonation = async () => {
    try {
      ...
      /* best-effort — restore the real session regardless */
    }
    endImpersonating()
    navigate('/')
  }
  ...
  const [navLayout, setNavLayout] = useState(() => {
    try {
      return localStorage.getItem('hs_nav_layout') ?? 'left'
    } catch {
      return 'left'
    }
  })
  ...
  const handleSearchKey = (e) => {
    const count = (
      searchQuery.length >= 1
        ? SEARCH_DATA.filter(
            (d) => d.label.toLowerCase().includes(searchQuery.toLowerCase()) || d.sub.toLowerCase().includes(searchQuery.toLowerCase()),
          ).slice(0, 8)
        : SEARCH_DATA.filter((d) => d.type === 'page').slice(0, 6)
    ).length
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      setSearchActiveIdx((i) => Math.min(i + 1, count - 1))
    }
    if (e.key === 'ArrowUp') {
      e.preventDefault()
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Snackbar open={warnSeconds !== null} anchorOrigin={{ vertical: 'top', horizontal: 'center' }} sx={{ top: '72px !important' }}>
        <Alert
          severity="warning"
          variant="filled"
          action={
        ...
        >
          ...
          <Box
            sx={{
              ...
              right: 0,
              height: 3,
              background: `linear-gradient(90deg, ${TEAL}, ${TEAL_LIGHT}, #0F9D58)`,
            }}
          />

          <Toolbar
            sx={{ minHeight: `${headerHeight}px !important`, gap: 1, px: { xs: 2, sm: 3 }, justifyContent: 'space-between', mt: '3px' }}
          >
          ...
          <MenuItem onClick={toggleLayout} sx={{ borderRadius: 2, py: 1, px: 1.5, '&:hover': { bgcolor: alpha(theme.palette.primary.main, 0.06) } }}>
            <ListItemIcon>
              {isTopNav ? (
                ...
                <ViewStreamRoundedIcon fontSize="small" sx={{ color: TEAL }} />
              )}
            </ListItemIcon>
            <Typography variant="body2" fontWeight={600}>
