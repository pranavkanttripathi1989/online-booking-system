...
const CREATE_ROOM_MUTATION = gql`
  mutation CreateRoom($input: RoomInput!) {
    createRoom(input: $input) { id name capacity is_active clinic { id name } }
  }
`
const UPDATE_ROOM_MUTATION = gql`
  mutation UpdateRoom($id: ID!, $input: RoomInput!) {
    updateRoom(id: $id, input: $input) { id name capacity is_active }
  }
`
const DELETE_ROOM_MUTATION = gql`
  mutation DeleteRoom($id: ID!) { deleteRoom(id: $id) }
`
...
function RoomRow({ room, onSave, onDelete }) {
  ...
  const handleSave = () => { onSave(room.id, vals); setEditing(false) }
      ...
      {editing ? (
        ...
        <>
          <Box flex={2}>
            ...
            <Typography variant="caption" color="text.secondary">Capacity: {room.capacity ?? 1}</Typography>
          </Box>
          <Chip
          ...
          <Stack direction="row" spacing={0.25}>
            ...
            <Tooltip title="Delete"><IconButton size="small" color="error" onClick={() => onDelete(room.id)}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
          </Stack>
        </>
      )}
    </Stack>
...
export default function RoomsManager() {
  ...
  const [createRoom] = useMutation(CREATE_ROOM_MUTATION, {
    onCompleted: () => { enqueueSnackbar('Room created', { variant: 'success' }); refetch(); setAdding(false) },
  ...
  const [deleteRoom] = useMutation(DELETE_ROOM_MUTATION, {
    onCompleted: () => { enqueueSnackbar('Room deleted', { variant: 'info' }); refetch() },
    onError: (e) => enqueueSnackbar(e.message, { variant: 'error' }),
  })
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-end" mb={3}>
        <Box>
          <Typography variant="h6" fontWeight={800}>Rooms</Typography>
      ...
      {/* Clinic filter */}
      <TextField
        select size="small" label="Filter by Clinic" value={filterClinic} sx={{ mb: 3, minWidth: 220 }}
      ...
      <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, overflow: 'hidden' }}>
        {loading ? (
          <Stack p={2} spacing={1}>
            {[...Array(4)].map((_,i) => <Skeleton key={i} variant="rounded" height={52} sx={{borderRadius:1.5}} />)}
          </Stack>
        ) : rooms.length === 0 ? (
          ...
          rooms.map((room, idx) => (
            <Box key={room.id}>
              <RoomRow
                room={room}
                onSave={handleSave}
                onDelete={(id) => deleteRoom({ variables: { id } })}
              />
        ...
        {adding && (
          <>
            <Divider />
            <AddRoomRow
              clinicId={filterClinic || clinics[0]?.id}
              onAdd={handleAdd}
