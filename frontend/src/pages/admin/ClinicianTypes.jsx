...
const GET_CLINICIAN_TYPES = gql`
  query GetClinicianTypes {
    clinicianTypes {
      id
      name
      description
...
`
const CREATE_CT = gql`
  mutation CreateClinicianType($input: CreateClinicianTypeInput!) {
    createClinicianType(input: $input) {
      success
      userErrors {
        message
      }
...
`
const UPDATE_CT = gql`
  mutation UpdateClinicianType($id: ID!, $input: UpdateClinicianTypeInput!) {
    updateClinicianType(id: $id, input: $input) {
      success
      userErrors {
        message
      }
...
`
const DELETE_CT = gql`
  mutation DeleteClinicianType($id: ID!) {
    deleteClinicianType(id: $id) {
      success
      userErrors {
        message
      }
...
export default function AdminClinicianTypes() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { data } = await client.query({ query: GET_CLINICIAN_TYPES, fetchPolicy: 'network-only' })
      setTypes(data?.clinicianTypes || [])
    } catch (err) {
      setTypes(MOCK_CLINICIAN_TYPES)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleToggle = async (item) => {
    try {
      await client.mutate({ mutation: UPDATE_CT, variables: { id: item.id, input: { is_active: !item.is_active } } })
      load()
    } catch (err) {
      setFormError(err.message)
    }
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Define clinician specialisation types used in rooms and scheduling
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      {showForm && (
        <Card sx={{ mb: 3, p: 2 }}>
          <Typography variant="h6" fontWeight={600} mb={2}>
            {editItem ? 'Edit Type' : 'New Clinician Type'}
          </Typography>
      ...
      <ConfirmDialog
        isOpen={confirmOpen}
        title="Delete Clinician Type"
        message="Delete this clinician type? This may affect rooms using it."
