...
import {
  Alert, Box, Button, Card, CircularProgress,
  Grid, IconButton, MenuItem, Stack, TextField, Typography, Tooltip,
} from '@mui/material'
...
const GET_DEPARTMENTS = gql`query GetDepartments { departments { id name clinic { id name } } }`
const CREATE_DEPARTMENT = gql`mutation CreateDepartment($input: DepartmentInput!) { createDepartment(input: $input) { id name clinic { id name } } }`
const UPDATE_DEPARTMENT = gql`mutation UpdateDepartment($id: ID!, $input: DepartmentInput!) { updateDepartment(id: $id, input: $input) { id name clinic { id name } } }`
const DELETE_DEPARTMENT = gql`mutation DeleteDepartment($id: ID!) { deleteDepartment(id: $id) { success userErrors { message } } }`
...
export default function AdminDepartments() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.clinic_id) { setFormError('Clinic is required'); return }
    setSubmitting(true)
    setFormError(null)
    try {
      if (editItem) {
    ...
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Specialty groupings (Cardiology, Dental, Physio) for clinicians and services</Typography>
        </Box>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => { reset(); setShowForm((p) => !p) }}>Add Department</Button>
      </Stack>
      ...
      {showForm && (
        <Card sx={{ mb: 3, p: 2 }}>
          <Typography variant="h6" fontWeight={600} mb={2}>{editItem ? 'Edit Department' : 'New Department'}</Typography>
          <Box component="form" onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth required size="small" label="Name" value={form.name} onChange={(e) => setField('name', e.target.value)} />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth required size="small" label="Clinic" value={form.clinic_id} onChange={(e) => setField('clinic_id', e.target.value)}>
                  {clinics.map((c) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}
                </TextField>
              </Grid>
              <Grid item xs={12}>
                <Stack direction="row" spacing={1}>
                  <Button type="submit" variant="contained" disabled={submitting}>{editItem ? 'Update' : 'Create'}</Button>
            ...
            <Box component="tbody">
              ...
              {departments.map((item) => (
                <Box component="tr" key={item.id} sx={{ '&:hover': { bgcolor: 'grey.50' }, borderBottom: '1px solid', borderColor: 'divider' }}>
                  ...
                  <Box component="td" sx={{ px: 2, py: 1.5 }}><Typography variant="body2" color="text.secondary">{item.clinic?.name ?? '—'}</Typography></Box>
                  <Box component="td" sx={{ px: 2, py: 1.5 }}>
                    <Stack direction="row" spacing={0.5}>
                      <Tooltip title="Edit">
                        <IconButton size="small" onClick={() => { setEditItem(item); setForm({ name: item.name, clinic_id: item.clinic?.id ?? '' }); setShowForm(true) }}>
                          <EditIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Delete">
                        <IconButton size="small" color="error" onClick={() => { setDeletingId(item.id); setConfirmOpen(true) }}>
      ...
      <ConfirmDialog isOpen={confirmOpen} title="Delete Department" message="Delete this department? Clinicians and services assigned to it will keep their other data but lose this grouping." onConfirm={confirmDelete} onCancel={() => { setConfirmOpen(false); setDeletingId(null) }} />
    </Box>
  )
}
