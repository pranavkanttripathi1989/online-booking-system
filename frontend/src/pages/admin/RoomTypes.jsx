...
const GET_ROOM_TYPES = gql`query GetRoomTypes { roomTypes { id name description is_active } }`
const CREATE_RT = gql`mutation CreateRoomType($input: CreateRoomTypeInput!) { createRoomType(input:$input) { success userErrors{message} } }`
...
export default function AdminRoomTypes() {
  ...
  const handleToggle = async (item) => {
    try { await client.mutate({ mutation: UPDATE_RT, variables: { id: item.id, input: { is_active: !item.is_active } } }); load() }
    catch (err) { setFormError(err.message) }
  }
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Room Types</Typography>
          <Typography variant="body2" color="text.secondary">Define room categories used when creating clinic rooms</Typography>
        </Box>
      ...
      {showForm && (
        <Card sx={{ mb: 3, p: 2 }}>
          <Typography variant="h6" fontWeight={600} mb={2}>{editItem ? 'Edit Room Type' : 'New Room Type'}</Typography>
      ...
      <ConfirmDialog isOpen={confirmOpen} title="Delete Room Type" message="Delete this room type? This may affect rooms using it." onConfirm={confirmDelete} onCancel={() => { setConfirmOpen(false); setDeletingId(null) }} />
    </Box>
  )
}
