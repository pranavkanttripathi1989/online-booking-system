...
const RESOLVE_RIGHTS_REQUEST = gql`
  mutation ResolveRightsRequest($id: ID!, $input: ResolveRightsRequestInput!) {
    resolveRightsRequest(id: $id, input: $input) {
      id
      status
      resolved_at
    }
...
export default function AdminRightsRequests() {
  ...
  const submitResolve = async () => {
    ...
    try {
      await client.mutate({
        mutation: RESOLVE_RIGHTS_REQUEST,
        variables: { id: resolving.id, input: { status: resolveStatus, notes: resolveNotes || undefined } },
      })
    ...
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3} flexWrap="wrap" gap={1}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            DPDP access / correction / erasure requests — review and resolve by hand
          </Typography>
        </Box>
        <TextField
          select
                ...
                {requests.map((r) => (
                  ...
                  >
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      <Typography fontWeight={600}>{TYPE_LABEL[r.type] ?? r.type}</Typography>
                    </Box>
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                    ...
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      <Typography
                        variant="body2"
                        color={isOverdue(r) ? 'error.main' : 'text.primary'}
                        fontWeight={isOverdue(r) ? 700 : 400}
                      >
                        {new Date(r.sla_due_at).toLocaleDateString('en-IN')}
                        {isOverdue(r) ? ' (overdue)' : ''}
                      </Typography>
                    </Box>
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                    ...
                    <Box component="td" sx={{ px: 2, py: 1.5, maxWidth: 240 }}>
                    ...
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      {r.status === 'pending' && (
                        <Button size="small" variant="outlined" onClick={() => openResolve(r)}>
                          Resolve
                        </Button>
                      )}
                    </Box>
      ...
      <Dialog open={!!resolving} onClose={() => setResolving(null)} fullWidth maxWidth="sm">
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={submitting} onClick={submitResolve}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
