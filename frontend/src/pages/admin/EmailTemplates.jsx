...
const MOCK_EMAIL_TEMPLATES = [
  {
    ...
    subject: 'Your appointment is confirmed — {{patient_name}}',
    body: 'Dear {{patient_name}},\n\nYour appointment with {{clinician_name}} on {{date}} at {{time}} has been confirmed.\n\nLocation: {{clinic_name}}\n\nThank you,\nHealthSync Team',
    variables: ['patient_name', 'clinician_name', 'date', 'time', 'clinic_name'],
    is_active: true,
  },
  {
    id: 'et2',
    ...
    subject: 'Reminder: Your appointment tomorrow — {{patient_name}}',
    body: 'Dear {{patient_name}},\n\nThis is a reminder that you have an appointment tomorrow with {{clinician_name}} at {{time}}.\n\nThank you,\nHealthSync Team',
    variables: ['patient_name', 'clinician_name', 'time'],
    is_active: true,
  },
  {
    id: 'et3',
  ...
  },
  {
    ...
    name: 'Welcome Email',
    type: 'welcome',
    subject: 'Welcome to HealthSync, {{name}}!',
...
export default function AdminEmailTemplates() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { data } = await client.query({ query: GET_EMAIL_TEMPLATES, fetchPolicy: 'network-only' })
    ...
    <Box>
      <Box mb={3}>
        ...
        <Typography variant="body2" color="text.secondary">
          Edit system email templates sent to users and clinicians
        </Typography>
      </Box>
              ...
              {editItem?.id !== t.id ? (
                ...
                <Box>
                  <Typography variant="h6" fontWeight={600} mb={2}>
                    Editing: {t.name || TYPE_LABELS[t.type] || t.type}
                  </Typography>
                  {formError && (
      ...
      <Dialog open={!!previewItem} onClose={() => setPreviewItem(null)} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          ...
          <Box sx={{ p: 1.5, bgcolor: 'grey.100', borderRadius: 1, mb: 2, fontFamily: 'monospace' }}>{previewItem?.subject}</Box>
          <Divider sx={{ mb: 2 }} />
          <Typography variant="subtitle2" gutterBottom>
            Body:
          </Typography>
