...
const GET_LANGUAGES = gql`query GetLanguages { languages { id name code is_active is_default } }`
const CREATE_LANG = gql`mutation CreateLanguage($input: CreateLanguageInput!) { createLanguage(input:$input) { success userErrors{message} } }`
const UPDATE_LANG = gql`mutation UpdateLanguage($id:ID!,$input: UpdateLanguageInput!) { updateLanguage(id:$id,input:$input) { success userErrors{message} } }`
const DELETE_LANG = gql`mutation DeleteLanguage($id:ID!) { deleteLanguage(id:$id) { success userErrors{message} } }`
...
export default function AdminLanguages() {
  ...
  const load = async () => {
    setLoading(true)
    try { const { data } = await client.query({ query: GET_LANGUAGES, fetchPolicy: 'network-only' }); setLanguages(data?.languages || []) }
    catch (err) { setLanguages(MOCK_LANGUAGES) }
  ...
  const handleToggle = async (item) => {
    ...
    try { await client.mutate({ mutation: UPDATE_LANG, variables: { id: item.id, input: { is_active: !item.is_active } } }); load() }
    catch (err) { setFormError(err.message) }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showSuccess('Language deleted.'); load()
    } catch (err) { setFormError(err.message) }
    setDeletingId(null)
  }
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Languages</Typography>
          <Typography variant="body2" color="text.secondary">Configure supported languages for the application</Typography>
        </Box>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => { reset(); setShowForm(p => !p) }}>Add Language</Button>
      </Stack>
      ...
      {showForm && (
        <Card sx={{ mb: 3, p: 2 }}>
          <Typography variant="h6" fontWeight={600} mb={2}>{editItem ? 'Edit Language' : 'New Language'}</Typography>
          <Box component="form" onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}><TextField fullWidth required size="small" label="Language Name" placeholder="e.g. English" value={form.name} onChange={e => setField('name', e.target.value)} /></Grid>
              <Grid item xs={12} sm={6}><TextField fullWidth required size="small" label="Locale Code" placeholder="e.g. en, fr, de" value={form.code} onChange={e => setField('code', e.target.value.toLowerCase())} /></Grid>
              <Grid item xs={12}>
      ...
      <Card>
        <Box sx={{ overflowX: 'auto' }}>
          <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <Box component="tbody">
              {languages.length === 0 && (
                <Box component="tr"><Box component="td" colSpan={5} sx={{ textAlign: 'center', py: 6 }}>
                ...
                <Box component="tr" key={item.id} sx={{ '&:hover': { bgcolor: 'grey.50' }, borderBottom: '1px solid', borderColor: 'divider' }}>
                  ...
                  <Box component="td" sx={{ px: 2, py: 1.5 }}>
                    <Tooltip title={item.is_default && item.is_active ? 'Cannot deactivate the default language' : ''}>
                      <Stack direction="row" alignItems="center" spacing={1} sx={{ display: 'inline-flex' }}>
                        <Switch size="small" checked={!!item.is_active} onChange={() => handleToggle(item)} disabled={item.is_default && item.is_active} />
                  ...
                  <Box component="td" sx={{ px: 2, py: 1.5 }}>
                    <Stack direction="row" spacing={0.5}>
                      <Tooltip title="Edit">
                        <IconButton size="small" onClick={() => { setEditItem(item); setForm({ name: item.name, code: item.code, is_active: item.is_active, is_default: item.is_default }); setShowForm(true) }}>
                          <EditIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title={canDelete(item) ? 'Delete' : 'Cannot delete the default language'}>
                        <span>
                          <IconButton size="small" color="error" disabled={!canDelete(item)} onClick={() => { setDeletingId(item.id); setConfirmOpen(true) }}>
      ...
      <ConfirmDialog isOpen={confirmOpen} title="Delete Language" message="Delete this language? This will remove it from all language selections." onConfirm={confirmDelete} onCancel={() => { setConfirmOpen(false); setDeletingId(null) }} />
    </Box>
  )
}
