...
const QUEUE_BOARD_QUERY = gql`
  query QueueBoardDisplay($clinician_id: ID!) {
    queueBoard(clinician_id: $clinician_id) {
      clinician_name
      now_serving { token_no patient_name }
      waiting { token_no patient_name }
    }
  }
`
function QueueDisplay() {
  ...
  if (loading && !board) return <Box sx={{ display: 'flex', justifyContent: 'center', p: 8 }}><CircularProgress size={64} /></Box>
  ...
  return (
    <Box sx={{ bgcolor: '#0b1220', color: '#fff', minHeight: '100vh', p: { xs: 3, md: 6 } }}>
      <Typography variant="h4" fontWeight={700} mb={4} textAlign="center">{board?.clinician_name}</Typography>

      <Box sx={{ textAlign: 'center', mb: 6 }}>
        <Typography variant="h6" color="grey.400">NOW SERVING</Typography>
        {board?.now_serving ? (
          ...
          <Typography variant="h3" color="grey.500">—</Typography>
        )}
      </Box>

      <Typography variant="h6" color="grey.400" textAlign="center" mb={2}>UP NEXT</Typography>
      <Stack direction="row" spacing={3} justifyContent="center" flexWrap="wrap">
        {(board?.waiting ?? []).slice(0, 5).map((e, i) => (
          <Box key={i} sx={{ textAlign: 'center', minWidth: 120 }}>
            ...
            <Typography variant="body1" color="grey.400">{e.patient_name}</Typography>
          </Box>
        ))}
        {(board?.waiting ?? []).length === 0 && <Typography variant="h5" color="grey.600">Queue is empty</Typography>}
      </Stack>
    </Box>
  )
}
