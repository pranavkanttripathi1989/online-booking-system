...
const GET_MY_PROFILE = gql`
  query GetMyProfile {
    myProfile {
      id first_name last_name email user_image phone phone_country_code
      address_line1 address_line2 city postal_code country
      is_active created_at updated_at
      role   { id name description }
      clinic { id name address phone email }
      clinician { id firstName lastName email phone }
      patient   { id firstName lastName dateOfBirth email }
      clientOrg { id name code contactEmail }
    }
  }
`
const UPDATE_PROFILE = gql`
  mutation UpdateProfile($input: UpdateProfileInput!) {
    updateProfile(input: $input) {
      ...
      profile {
        id first_name last_name email user_image phone phone_country_code
        address_line1 address_line2 city postal_code country is_active updated_at
        role { id name }
        clinic { id name }
      }
    }
...
const UPLOAD_IMAGE = gql`
  mutation UploadProfileImage($imageBase64: String!, $filename: String) {
    uploadProfileImage(imageBase64: $imageBase64, filename: $filename) {
      success userErrors { message }
      profile { id user_image first_name last_name email }
    }
  }
`
...
export default function ProfilePage() {
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      ...
      if (r?.updateProfile?.userErrors?.length) { setError(r.updateProfile.userErrors[0].message); return }
  ...
  const handleFileChange = async (e) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (r?.uploadProfileImage?.userErrors?.length) { setError(r.uploadProfileImage.userErrors[0].message); return }
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      if (r?.deleteProfileImage?.userErrors?.length) { setError(r.deleteProfileImage.userErrors[0].message); return }
      if (r?.deleteProfileImage?.success) { setProfile(prev => ({ ...prev, user_image: null })); setImageUrl(null) }
    } catch (err) { setError(err.message) }
    ...
    <Box>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={4}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Manage your account information and security settings</Typography>
        </Box>
        {!editing && (
          <Button variant="contained" startIcon={<EditIcon />} onClick={() => { setEditing(true); setError(null); setEditTab(0) }}>
            Edit Profile
          </Button>
        )}
      </Stack>
              ...
              <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={3}>
                <Stack direction="row" spacing={2} alignItems="center">
                  <Box sx={{
                    ...
                    fontSize: 28, fontWeight: 700, flexShrink: 0, border: '2px solid', borderColor: 'divider',
                  }}>
                    {imageUrl
              ...
              <Grid container spacing={3}>
                ...
                {profile.clientOrg && (
                  <Grid item xs={12}>
                    ...
                    <Typography fontWeight={600}>{profile.clientOrg.name} ({profile.clientOrg.code})</Typography>
                  </Grid>
                )}
              </Grid>
            </CardContent>
          ...
          {(profile.address_line1 || profile.city || profile.country) && (
            <Card>
              <CardContent sx={{ p: 3 }}>
                ...
                <Grid container spacing={2}>
                  {profile.address_line1 && <Grid item xs={12}><Typography variant="caption" color="text.secondary">Address</Typography><Typography>{profile.address_line1}{profile.address_line2 ? `, ${profile.address_line2}` : ''}</Typography></Grid>}
                  <Grid item xs={6}><Typography variant="caption" color="text.secondary">City</Typography><Typography>{profile.city || '—'}</Typography></Grid>
                  <Grid item xs={6}><Typography variant="caption" color="text.secondary">Postal Code</Typography><Typography>{profile.postal_code || '—'}</Typography></Grid>
                  <Grid item xs={12}><Typography variant="caption" color="text.secondary">Country</Typography><Typography>{profile.country || '—'}</Typography></Grid>
                </Grid>
              </CardContent>
            </Card>
          )}
          ...
          <CardContent sx={{ p: 3 }}>
              ...
              <Box component="form" onSubmit={handleProfileSave}>
                ...
                <Stack direction="row" alignItems="center" spacing={2} mb={3}>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap">
                    ...
                    {imageUrl && (
                      <Button variant="outlined" color="error" size="small" startIcon={<DeleteIcon />} disabled={uploading} onClick={handleDeleteImage}>
                        Remove
                      </Button>
                    )}
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}><TextField fullWidth required size="small" label="First Name" value={pForm.first_name} onChange={e => setPForm(p => ({ ...p, first_name: e.target.value }))} /></Grid>
                  <Grid item xs={12} sm={6}><TextField fullWidth required size="small" label="Last Name" value={pForm.last_name} onChange={e => setPForm(p => ({ ...p, last_name: e.target.value }))} /></Grid>
                  <Grid item xs={12}><TextField fullWidth size="small" label="Phone" placeholder="+44 7700 900000" value={pForm.phone} onChange={e => setPForm(p => ({ ...p, phone: e.target.value }))} /></Grid>
                </Grid>

                <Divider sx={{ my: 3 }} />
                <Typography variant="h6" fontWeight={700} mb={2}>Address</Typography>
                <Grid container spacing={2}>
                  <Grid item xs={12}><TextField fullWidth size="small" label="Address Line 1" value={pForm.address_line1} onChange={e => setPForm(p => ({ ...p, address_line1: e.target.value }))} /></Grid>
                  <Grid item xs={12}><TextField fullWidth size="small" label="Address Line 2" value={pForm.address_line2} onChange={e => setPForm(p => ({ ...p, address_line2: e.target.value }))} /></Grid>
                  <Grid item xs={6}><TextField fullWidth size="small" label="City" value={pForm.city} onChange={e => setPForm(p => ({ ...p, city: e.target.value }))} /></Grid>
                  <Grid item xs={6}><TextField fullWidth size="small" label="Postal Code" value={pForm.postal_code} onChange={e => setPForm(p => ({ ...p, postal_code: e.target.value }))} /></Grid>
                  <Grid item xs={12}><TextField fullWidth size="small" label="Country" value={pForm.country} onChange={e => setPForm(p => ({ ...p, country: e.target.value }))} /></Grid>
                </Grid>
            ...
            {editTab === 1 && (
              <Box component="form" onSubmit={handlePasswordSave}>
                <Grid container spacing={2}>
                  <Grid item xs={12}><TextField fullWidth required size="small" type="password" label="Current Password" value={pwForm.current_password} onChange={e => setPwForm(p => ({ ...p, current_password: e.target.value }))} /></Grid>
                  <Grid item xs={12}><TextField fullWidth required size="small" type="password" label="New Password" helperText="Minimum 8 characters" value={pwForm.new_password} onChange={e => setPwForm(p => ({ ...p, new_password: e.target.value }))} /></Grid>
                  <Grid item xs={12}><TextField fullWidth required size="small" type="password" label="Confirm New Password" value={pwForm.confirm_password} onChange={e => setPwForm(p => ({ ...p, confirm_password: e.target.value }))} /></Grid>
                  <Grid item xs={12}>
                    <Stack direction="row" spacing={1}>
                      <Button type="submit" variant="contained" disabled={saving}>{saving ? 'Saving…' : 'Change Password'}</Button>
                      <Button variant="outlined" onClick={() => { setEditing(false); setError(null) }}>Cancel</Button>
                    </Stack>
                  </Grid>
                </Grid>
