...
const batchesMock = (batches = []) => ({
  request: { query: GET_BATCHES, variables: { clinic_id: undefined } },
  result: { data: { drugBatches: batches.map(batchToGraphQL) } },
})
...
describe('manager/pharmacy (REQ059)', () => {
  ...
  it('Drug Catalog: creating a drug calls the real mutation and refreshes the list', async () => {
    const mocks = [
      ...
      batchesMock(),
      {
        ...
        result: { data: { createDrug: { __typename: 'Drug', id: 'drug-new' } } },
      },
  ...
  it("Dispense: the batch picker only offers batches matching the selected item's drug", async () => {
    ...
    const mocks = [
      ...
      batchesMock(batches),
      {
        request: { query: PATIENTS_QUERY, variables: { search: 'Test', first: 10 } },
        result: {
          data: {
            patients: {
              ...
              data: [patient],
              paginatorInfo: {
                __typename: 'PatientPaginatorInfo',
                count: 1,
                currentPage: 1,
                hasMorePages: false,
                lastPage: 1,
      ...
      {
        request: { query: GET_PATIENT_PRESCRIPTIONS, variables: { patient_id: 'pat-1' } },
        result: {
          data: {
  ...
  it('Dispense: defaults the batch picker to the earliest-expiring batch (FEFO)', async () => {
    ...
    const mocks = [
      ...
      {
        request: {
          query: DISPENSE_PRESCRIPTION_ITEM,
          variables: { input: { prescription_item_id: 'item-1', batch_id: 'batch-early', quantity: 2 } },
        },
        result: { data: { dispensePrescriptionItem: { __typename: 'DrugBatch', id: 'batch-early', quantity_remaining: 18 } } },
      },
      batchesMock(batches),
    ]
    renderPage(mocks)
  ...
  it('Dispense: a real dispensePrescriptionItem call is made with the right variables', async () => {
    ...
    const mocks = [
      ...
      },
      {
        request: {
          query: DISPENSE_PRESCRIPTION_ITEM,
          variables: { input: { prescription_item_id: 'item-1', batch_id: 'batch-match', quantity: 2 } },
        },
        result: { data: { dispensePrescriptionItem: { __typename: 'DrugBatch', id: 'batch-match', quantity_remaining: 48 } } },
      },
      batchesMock(batches),
    ]
    renderPage(mocks)
  ...
  it('Pending Dispense: lists items org-wide and dispenses directly from the queue', async () => {
    ...
    const mocks = [
      ...
      { request: { query: GET_PENDING_DISPENSE }, result: { data: { pendingDispenseItems: [pendingRow] } } },
      {
        request: {
          query: DISPENSE_PRESCRIPTION_ITEM,
          variables: { input: { prescription_item_id: 'item-9', batch_id: 'batch-match', quantity: 6 } },
        },
        result: { data: { dispensePrescriptionItem: { __typename: 'DrugBatch', id: 'batch-match', quantity_remaining: 44 } } },
      },
      batchesMock(batches),
    ...
    await waitFor(() => expect(screen.getByText('Dispensed.')).toBeInTheDocument())
    await waitFor(() =>
      expect(screen.getByText('Nothing pending — every issued prescription item has been fully dispensed.')).toBeInTheDocument(),
    )
  })
