...
const CREATE_INTAKE_FIELD = gql`
  mutation CreateIntakeFieldConfig($input: CreateIntakeFieldInput!) {
    createIntakeFieldConfig(input: $input) {
      success
      userErrors {
        message
      }
...
const UPDATE_INTAKE_FIELD = gql`
  mutation UpdateIntakeFieldConfig($id: ID!, $input: UpdateIntakeFieldInput!) {
    updateIntakeFieldConfig(id: $id, input: $input) {
      success
      userErrors {
        message
      }
...
function ClinicForms() {
  ...
  useEffect(() => {
    client
      .query({ query: GET_CLINIC_FORMS_CLINICS, fetchPolicy: 'network-only' })
      .then(({ data }) => {
  ...
  const loadChecklistItems = useCallback(() => {
    ...
    client
      .query({ query: CHECKLIST_ITEMS_QUERY, variables: { clinic_id: clinicId }, fetchPolicy: 'network-only' })
  ...
  const loadIntakeFields = useCallback(() => {
    ...
    client
      .query({ query: INTAKE_FIELD_CONFIGS_QUERY, variables: { clinic_id: clinicId }, fetchPolicy: 'network-only' })
  ...
  useEffect(() => {
    if (!clinicId) return
    client
      .query({ query: GET_CLINIC_FORMS_PRODUCTS, variables: { clinic_id: clinicId }, fetchPolicy: 'network-only' })
  ...
  const openCreateChecklistItem = () => {
    ...
    setChecklistForm(dfChecklistItem)
  ...
  const submitChecklistItem = async () => {
    ...
    try {
      if (editChecklistItem) {
        ...
        if (!data?.updateChecklistItem?.success)
          throw new Error(data?.updateChecklistItem?.userErrors?.[0]?.message ?? 'Failed to update checklist item')
        ...
        const { data } = await client.mutate({
          ...
          variables: {
            input: {
              clinic_id: clinicId,
              product_id: checklistForm.product_id || undefined,
              label: checklistForm.label,
              is_required: checklistForm.is_required,
              sort_order: Number(checklistForm.sort_order) || 0,
            },
        ...
        if (!data?.createChecklistItem?.success)
          throw new Error(data?.createChecklistItem?.userErrors?.[0]?.message ?? 'Failed to create checklist item')
  ...
  // ── Intake field CRUD ─────────────────────────────────────────────────────
  const openCreateIntakeField = () => {
    setEditIntakeField(null)
    setIntakeForm(dfIntakeField)
    ...
    try {
      if (editIntakeField) {
        const { data } = await client.mutate({
          ...
          variables: {
            id: editIntakeField.id,
            input: {
              label: intakeForm.label,
              field_type: intakeForm.field_type,
              is_required: intakeForm.is_required,
              sort_order: Number(intakeForm.sort_order) || 0,
            },
          },
        })
        if (!data?.updateIntakeFieldConfig?.success)
          throw new Error(data?.updateIntakeFieldConfig?.userErrors?.[0]?.message ?? 'Failed to update intake field')
        ...
        const { data } = await client.mutate({
          ...
          variables: {
            input: {
              clinic_id: clinicId,
              product_id: intakeForm.product_id || undefined,
              key: intakeForm.key,
              label: intakeForm.label,
              field_type: intakeForm.field_type,
              is_required: intakeForm.is_required,
        ...
        if (!data?.createIntakeFieldConfig?.success)
          throw new Error(data?.createIntakeFieldConfig?.userErrors?.[0]?.message ?? 'Failed to create intake field')
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (type === 'checklist') {
        ...
        if (!data?.deleteChecklistItem?.success)
          throw new Error(data?.deleteChecklistItem?.userErrors?.[0]?.message ?? 'Failed to delete checklist item')
        showSuccess('Checklist item deleted.')
        loadChecklistItems()
      } else {
        const { data } = await client.mutate({ mutation: DELETE_INTAKE_FIELD, variables: { id } })
        if (!data?.deleteIntakeFieldConfig?.success)
          throw new Error(data?.deleteIntakeFieldConfig?.userErrors?.[0]?.message ?? 'Failed to delete intake field')
  ...
  return (
    <Box>
      ...
      {!clinicId && <Alert severity="info">Select a clinic to view or configure its forms.</Alert>}
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
              ...
              <Grid item xs={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    value={checklistForm.product_id}
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) => setChecklistForm((p) => ({ ...p, sort_order: e.target.value }))}
                inputProps={{ min: 0 }}
              />
            </Grid>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                size="small"
                label="Label"
                placeholder="e.g. List any allergies"
                value={intakeForm.label}
                onChange={(e) => setIntakeForm((p) => ({ ...p, label: e.target.value }))}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth size="small">
            ...
            <Grid item xs={6}>
              <TextField
                ...
                type="number"
                label="Sort Order"
                value={intakeForm.sort_order}
                onChange={(e) => setIntakeForm((p) => ({ ...p, sort_order: e.target.value }))}
                inputProps={{ min: 0 }}
              />
            </Grid>
            <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center' }}>
              <FormControlLabel
                control={
                  <Checkbox
                    checked={intakeForm.is_required}
