...
const GET_RESOURCES = gql`
  query GetResources($clinic_id: ID) {
    resources(clinic_id: $clinic_id) {
      id name type is_bookable
      clinic { id name }
    }
  }
`
const GET_CLINICS = gql`
  query GetResourcesClinics { clinics { id name } }
`
const CREATE_RESOURCE = gql`
  mutation CreateResource($input: ResourceInput!) {
    createResource(input: $input) { id }
  }
`
const UPDATE_RESOURCE = gql`
  mutation UpdateResource($id: ID!, $input: ResourceInput!) {
    updateResource(id: $id, input: $input) { id }
  }
`
const DELETE_RESOURCE = gql`
  mutation DeleteResource($id: ID!) {
    deleteResource(id: $id) { success userErrors { message } }
  }
`
...
function ManagerResources() {
  ...
  useEffect(() => {
    client.query({ query: GET_CLINICS }).then(({ data }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!res?.deleteResource?.success) { setFormError(res?.deleteResource?.userErrors?.[0]?.message || 'Delete failed'); return }
      showSuccess('Resource deleted.'); loadResources()
    } catch (err) { setFormError(err.message) }
    setDeletingId(null)
  }
  ...
  return (
    <Box>
      ...
      {showForm && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" fontWeight={600} mb={2}>{editingResource ? 'Edit Resource' : 'New Resource'}</Typography>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required size="small">
                    ...
                    <Select label="Type" value={form.type} onChange={e => setField('type', e.target.value)}>
                      {RESOURCE_TYPES.map(t => <MenuItem key={t} value={t}>{TYPE_LABELS[t]}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select label="Bookable" value={form.isBookable ? 'yes' : 'no'} onChange={e => setField('isBookable', e.target.value === 'yes')}>
                ...
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1}>
                    ...
                    <Stack direction="row" spacing={0.5}>
                      <Tooltip title="Edit">
                        <IconButton size="small" aria-label={`Edit resource ${resource.name}`} onClick={() => handleEdit(resource)}><EditIcon fontSize="small" /></IconButton>
                      </Tooltip>
                      <Tooltip title="Delete">
                        <IconButton size="small" color="error" aria-label={`Delete resource ${resource.name}`} onClick={() => handleDelete(resource.id)}><DeleteIcon fontSize="small" /></IconButton>
                      </Tooltip>
                    </Stack>
                  </Stack>

                  <Typography variant="h6" fontWeight={700} noWrap sx={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{resource.name}</Typography>
                  <Typography variant="body2" color="text.secondary">{TYPE_LABELS[resource.type] || resource.type}</Typography>
                  <Typography variant="caption" color="text.secondary">{resource.clinic?.name}</Typography>
...
export default function ManagerResourcesWithBoundary() {
  return <ErrorBoundary><ManagerResources /></ErrorBoundary>
}
