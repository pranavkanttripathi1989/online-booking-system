      ...
      <Box sx={{ display:'flex', alignItems:'center', gap:1.5, mb:3, flexWrap:'wrap' }}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="contained" startIcon={loading ? <CircularProgress size={16} color="inherit" /> : <SaveRoundedIcon />}
            onClick={() => { if(validate()) createProduct({ variables:{ input:{ name:form.name, description:form.description||undefined, price:form.price?parseFloat(form.price):undefined, stock_quantity:form.stock_quantity?parseInt(form.stock_quantity):undefined, sku:form.sku||undefined, is_active:form.is_active } } }) }}
            disabled={loading} sx={{ borderRadius:2.5, textTransform:'none', fontWeight:700, bgcolor:'#8430CE','&:hover':{bgcolor:'#6A27A8'} }}>
