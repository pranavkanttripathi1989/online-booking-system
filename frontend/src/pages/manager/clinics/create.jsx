...
import {
  ...
  Grid, IconButton, MenuItem, Paper, Stack,
...
const INITIAL = { name:'',address:'',city:'',postcode:'',state:'',gstin:'',phone:'',email:'',timezone:'Europe/London',is_active:true }
...
export default function CreateClinicPage() {
  ...
  const [createClinic, { loading }] = useMutation(CREATE_CLINIC_MUTATION, {
    onCompleted: (d) => { enqueueSnackbar('Clinic created', { variant:'success' }); navigate(`/manager/clinics/${d.createClinic.id}`) },
  ...
  return (
    <Box className="page-enter">
      <Helmet><title>New Clinic — MediBook</title></Helmet>
      <Box sx={{ display:'flex', alignItems:'center', gap:1.5, mb:3, flexWrap:'wrap' }}>
        <IconButton onClick={() => navigate('/manager/clinics')} sx={{ bgcolor:'#F1F3F4','&:hover':{bgcolor:'#E8EAED'} }}><ArrowBackRoundedIcon /></IconButton>
        <Box sx={{ display:'flex', alignItems:'center', gap:1.5, flex:1 }}>
          ...
          <Box>
            <Typography variant="h5" fontWeight={800}>New Clinic</Typography>
            <Typography variant="body2" color="text.secondary">Add a new clinic location</Typography>
          </Box>
        </Box>
        <Stack direction="row" spacing={1}>
          <Button variant="outlined" onClick={() => navigate('/manager/clinics')} sx={{ borderRadius:2.5, textTransform:'none', fontWeight:700 }}>Cancel</Button>
        ...
        <Grid item xs={12} md={8}>
          <Paper elevation={0} sx={{ p:3, borderRadius:3, border:'1px solid #E8EAED', mb:3 }}>
            ...
            <Grid container spacing={2.5}>
              <Grid item xs={12}><TextField fullWidth label="Clinic Name *" value={form.name} onChange={set('name')} error={!!errors.name} helperText={errors.name} sx={{ '& .MuiOutlinedInput-root':{borderRadius:2} }} /></Grid>
