...
import {
  ...
  MenuItem, Paper, Skeleton, Stack, Switch, TextField, Typography,
} from '@mui/material'
...
export default function EditRoomPage() {
  ...
  const [updateRoom, { loading }] = useMutation(UPDATE_ROOM_MUTATION, {
    onCompleted: () => { enqueueSnackbar('Room updated', { variant:'success' }); navigate('/manager/rooms') },
  ...
  return (
    <Box className="page-enter">
      <Helmet><title>Edit Room — MediBook</title></Helmet>
      <Box sx={{ display:'flex', alignItems:'center', gap:1.5, mb:3, flexWrap:'wrap' }}>
        ...
        <Box sx={{ display:'flex', alignItems:'center', gap:1.5, flex:1 }}>
          ...
          <Box><Typography variant="h5" fontWeight={800}>Edit — {data?.room?.name}</Typography><Typography variant="body2" color="text.secondary">Update room details</Typography></Box>
        </Box>
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="contained" startIcon={loading ? <CircularProgress size={16} color="inherit" /> : <SaveRoundedIcon />}
            onClick={() => updateRoom({ variables:{ id, input:{ name:form.name, capacity:form.capacity?parseInt(form.capacity):undefined, clinic_id:form.clinic_id||undefined, is_active:form.is_active } } })}
