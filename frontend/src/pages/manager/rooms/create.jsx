...
import {
  ...
  Grid, IconButton, MenuItem, Paper, Stack, Switch, TextField, Typography,
} from '@mui/material'
...
export default function CreateRoomPage() {
  ...
  return (
    <Box className="page-enter">
      <Helmet><title>New Room — MediBook</title></Helmet>
      <Box sx={{ display:'flex', alignItems:'center', gap:1.5, mb:3, flexWrap:'wrap' }}>
        <IconButton onClick={() => navigate('/manager/rooms')} sx={{ bgcolor:'#F1F3F4' }}><ArrowBackRoundedIcon /></IconButton>
        <Box sx={{ display:'flex', alignItems:'center', gap:1.5, flex:1 }}>
          <Box sx={{ width:40, height:40, borderRadius:2.5, background:'linear-gradient(135deg,#E8F0FE,#C5D8FD)', display:'flex', alignItems:'center', justifyContent:'center' }}><MeetingRoomRoundedIcon sx={{ color:'#1A73E8', fontSize:'1.2rem' }} /></Box>
          <Box><Typography variant="h5" fontWeight={800}>New Room</Typography><Typography variant="body2" color="text.secondary">Add a room to a clinic</Typography></Box>
        </Box>
        <Stack direction="row" spacing={1}>
          <Button variant="outlined" onClick={() => navigate('/manager/rooms')} sx={{ borderRadius:2.5, textTransform:'none', fontWeight:700 }}>Cancel</Button>
          <Button variant="contained" startIcon={loading ? <CircularProgress size={16} color="inherit" /> : <SaveRoundedIcon />}
            onClick={() => { if(validate()) createRoom({ variables:{ input:{ name:form.name, capacity: form.capacity?parseInt(form.capacity):undefined, clinic_id:form.clinic_id||undefined, is_active:form.is_active } } }) }}
            disabled={loading} sx={{ borderRadius:2.5, textTransform:'none', fontWeight:700, background:'linear-gradient(135deg,#4285F4,#1A73E8)' }}>
