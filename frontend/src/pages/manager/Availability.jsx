...
const MOCK_AVAILABILITIES = [
  {
    id: 'mgrav-1',
    clinicianId: 'cln-1',
    clinician: { id: 'cln-1', firstName: 'Sarah', lastName: 'Mitchell' },
    clinicId: 'cli-1',
    clinic: { id: 'cli-1', name: 'Meridian Central' },
    roomId: 'rm-1',
    room: { id: 'rm-1', roomNumber: '1 — Consultation A' },
    startTime: '09:00',
    endTime: '17:00',
  ...
  },
  {
    id: 'mgrav-2',
    clinicianId: 'cln-2',
    clinician: { id: 'cln-2', firstName: 'James', lastName: 'Okafor' },
    clinicId: 'cli-1',
    clinic: { id: 'cli-1', name: 'Meridian Central' },
    roomId: null,
    room: null,
  ...
  },
  {
    id: 'mgrav-3',
    clinicianId: 'cln-3',
    clinician: { id: 'cln-3', firstName: 'Priya', lastName: 'Sharma' },
    clinicId: 'cli-4',
    clinic: { id: 'cli-4', name: 'CityCore West End' },
    roomId: 'rm-9',
    room: { id: 'rm-9', roomNumber: '9 — Cardio Suite' },
    startTime: '10:00',
    endTime: '18:00',
    recurrenceType: 'weekly',
    ...
    excludeSunday: false,
    validFrom: null,
    validUntil: null, // Always active
    isActive: true,
  },
  {
    id: 'mgrav-4',
    clinicianId: 'cln-5',
    clinician: { id: 'cln-5', firstName: 'Lucy', lastName: 'Harrington' },
    clinicId: 'cli-2',
    clinic: { id: 'cli-2', name: 'Meridian East' },
    roomId: 'rm-4',
    room: { id: 'rm-4', roomNumber: '4 — Physio Suite' },
    startTime: '08:30',
    endTime: '13:00',
    recurrenceType: 'weekly',
    dayOfWeek: 5, // Friday
  ...
  },
  {
    id: 'mgrav-5',
    clinicianId: 'cln-6',
    clinician: { id: 'cln-6', firstName: 'Ben', lastName: 'Whitfield' },
    clinicId: 'cli-3',
    clinic: { id: 'cli-3', name: 'Meridian North' },
    roomId: 'rm-6',
    room: { id: 'rm-6', roomNumber: '6 — Mental Health Suite' },
    startTime: '09:00',
    endTime: '17:00',
...
export default function ManagerAvailability() {
  ...
  const loadRoomsForClinic = useCallback(
    async (clinicId) => {
      if (!clinicId) {
        setRooms([])
        return
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.clinician_id) {
      setFormError('Please select a clinician.')
      return
    }
    if (!form.clinic_id) {
    ...
    if (form.mode !== 'slot' && (!form.capacity || parseInt(form.capacity, 10) < 1)) {
      setFormError('Session/hybrid mode needs a capacity of at least 1 token.')
      return
    }
    ...
    try {
      if (editingId) {
        ...
        if (res?.updateAvailability?.userErrors?.length) {
          setFormError(res.updateAvailability.userErrors[0].message)
          return
        }
        ...
        if (res?.createAvailability?.userErrors?.length) {
          setFormError(res.createAvailability.userErrors[0].message)
          return
        }
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth required size="small">
                      ...
                      <Select label="Clinician" value={form.clinician_id} onChange={(e) => setField('clinician_id', e.target.value)}>
                        {clinicians.map((c) => (
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth required size="small">
                      ...
                      <Select label="Scheduling Mode" value={form.mode} onChange={(e) => setField('mode', e.target.value)}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth required size="small">
                      ...
                      <Select label="Recurrence" value={form.recurrence_type} onChange={(e) => setField('recurrence_type', e.target.value)}>
                  ...
                  {form.recurrence_type === 'weekly' && (
                    <Grid item xs={12} sm={6}>
                      <FormControl fullWidth size="small">
                        ...
                        <Select label="Day of Week" value={form.day_of_week} onChange={(e) => setField('day_of_week', e.target.value)}>
                  ...
                  {form.recurrence_type === 'custom' && (
                    <Grid item xs={12}>
                      <TextField
                        fullWidth
                        size="small"
                        label="Custom Dates (comma-separated)"
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth size="small" disabled={!form.clinic_id || roomsLoading}>
                      ...
                      <Select label="Room (optional)" value={form.room_id} onChange={(e) => setField('room_id', e.target.value)}>
                ...
                {availabilities.map((avail) => (
                  ...
                  >
                    ...
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      ...
                      {avail.mode && avail.mode !== 'slot' && (
                        <Chip
                          label={
                            avail.mode === 'session'
                              ? `Session · ${avail.capacity ?? '?'} tokens`
                              : `Hybrid · ${avail.capacity ?? '?'} tokens`
                          }
                          size="small"
                          color="info"
                          sx={{ mt: 0.5 }}
                    ...
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      ...
                      {avail.excludeWeekends && <Chip label="No weekends" size="small" color="warning" sx={{ ml: 0.5 }} />}
                    </Box>
                    <Box component="td" sx={{ px: 2, py: 1.5 }}>
                      <Typography variant="caption" color="text.secondary">
                        {avail.validFrom
