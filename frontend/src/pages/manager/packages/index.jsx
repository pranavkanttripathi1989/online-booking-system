...
function ManagerPackages() {
  ...
  useEffect(() => {
    client
      .query({ query: GET_PACKAGE_CLINICS })
      .then(({ data }) => {
      ...
      .catch(() => {})
    client
      .query({ query: GET_PACKAGE_PRODUCTS })
      .then(({ data }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!r?.deletePackage?.success) {
        setFormError(r?.deletePackage?.userErrors?.[0]?.message || 'Delete failed')
        return
      }
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={4}>
                  <TextField
                    ...
                    inputProps={{ min: 1 }}
                    value={form.totalSittings}
                    onChange={(e) => setField('totalSittings', e.target.value)}
                  />
                </Grid>
                  ...
                  <Grid item xs={12}>
                    <FormControl fullWidth required size="small" disabled={!form.clinicId}>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setField('productIds', typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)
                        }
                        renderValue={(selected) => selected.map(productName).join(', ')}
                      >
                        {productsForClinic.length === 0 && (
                          <MenuItem value="" disabled>
                            No services for this clinic yet
                          </MenuItem>
                        )}
                        {productsForClinic.map((p) => (
              ...
              <Grid item xs={12} sm={6} md={4} lg={3} key={pkg.id}>
                <Card sx={{ height: '100%', '&:hover': { boxShadow: 4 }, transition: 'box-shadow 0.2s' }}>
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {pkg.total_sittings} sittings · valid {pkg.validity_days} days
                    </Typography>
                    {pkg.description && (
                      <Typography
                        variant="caption"
