...
function PrescriptionPrint() {
  ...
  return (
    <Box
      sx={{
        maxWidth: '210mm', mx: 'auto', p: 4, bgcolor: '#fff', color: '#000', position: 'relative',
    ...
    >
      ...
      {isReprint && (
        <Typography
          sx={{
            ...
            fontSize: '4rem', fontWeight: 900, color: 'rgba(200,0,0,0.15)', letterSpacing: 4,
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ position: 'relative', zIndex: 1 }}>
        <Box>
          ...
          <Typography variant="h6" fontWeight={700}>{clinic.name}</Typography>
        ...
        <Box textAlign="right">
          <Typography variant="body2" fontWeight={700}>{clinician.full_name}</Typography>
          {clinician.qualifications && <Typography variant="caption" display="block">{clinician.qualifications}</Typography>}
          {clinician.registration_number && <Typography variant="caption" display="block">Reg. No: {clinician.registration_number}</Typography>}
        </Box>
      </Stack>
      ...
      <Stack direction="row" justifyContent="space-between" sx={{ position: 'relative', zIndex: 1 }}>
        <Box>
          <Typography variant="body2"><strong>Patient:</strong> {patient.full_name}</Typography>
          {patient.date_of_birth && (
            <Typography variant="body2">
              <strong>DOB:</strong> {new Date(patient.date_of_birth).toLocaleDateString()}{patient.gender ? ` · ${patient.gender}` : ''}
            </Typography>
          )}
        </Box>
        <Typography variant="body2"><strong>Date:</strong> {new Date(prescription.issued_at).toLocaleDateString()}</Typography>
      </Stack>

      <Divider sx={{ my: 2 }} />

      <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, position: 'relative', zIndex: 1 }}>℞</Typography>
