    ...
    <Box className="page-enter">
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Internal follow-ups and staff to-dos
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      >
        {['All', ...TASK_STATUSES].map((s) => (
          <ToggleButton key={s} value={s} sx={{ textTransform: 'none', fontWeight: 700, px: 2 }}>
            {s}
          </ToggleButton>
        ))}
      </ToggleButtonGroup>
                ...
                <CardContent>
                  ...
                  {task.patient_name && (
                    <Stack
                      ...
                      spacing={0.75}
                      alignItems="center"
                      sx={{ mb: 0.5, cursor: task.patient_id ? 'pointer' : 'default' }}
                      onClick={() => task.patient_id && navigate(`/patients/${task.patient_id}`)}
                    >
        ...
        <DialogContent dividers>
          <Stack spacing={2} pt={0.5}>
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => <TextField {...field} label="Assigned to" fullWidth size="small" />}
            />
            <Controller
              name="patient_name"
              control={control}
              render={({ field }) => <TextField {...field} label="Related patient (optional)" fullWidth size="small" />}
            />
          </Stack>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setCreateOpen(false)} sx={{ textTransform: 'none', fontWeight: 700 }}>
            Cancel
          </Button>
          <Button
            variant="contained"
