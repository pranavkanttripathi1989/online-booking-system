...
import {
  ...
  DialogTitle, Divider, FormControl, Grid, IconButton, List,
  ListItemButton, ListItemText, MenuItem, Paper, Select, Stack, Table, TableBody,
...
const DRUGS_QUERY = gql`
  query SearchDrugs($search: String) { drugs(search: $search) { id name strength form } }
`
const PATIENT_PRESCRIPTIONS_QUERY = gql`
  query PatientPrescriptions($patient_id: ID!) {
    patientPrescriptions(patient_id: $patient_id) { id issued_at items { drug_name dose frequency } }
  }
`
const PRESCRIPTION_SETS_QUERY = gql`
  query PrescriptionSets { prescriptionSets { id name specialty } }
`
const REPEAT_PRESCRIPTION_QUERY = gql`
  query RepeatPrescription($source_id: ID!) {
    repeatPrescription(source_id: $source_id) {
      items { drug_id drug_name dose frequency route duration_days qty instructions substitutable }
    }
  }
`
const APPLY_SET_QUERY = gql`
  query ApplyPrescriptionSet($set_id: ID!) {
    applyPrescriptionSet(set_id: $set_id) {
      drug_id drug_name dose frequency route duration_days qty instructions substitutable
    }
  }
`
const CREATE_PRESCRIPTION = gql`
  mutation CreatePrescription($input: CreatePrescriptionInput!) { createPrescription(input: $input) { id } }
`
const CREATE_PRESCRIPTION_SET = gql`
  mutation CreatePrescriptionSet($input: CreatePrescriptionSetInput!) { createPrescriptionSet(input: $input) { id } }
`
...
function PrescriptionBuilder() {
  ...
  const { data: historyData } = useQuery(PATIENT_PRESCRIPTIONS_QUERY, { variables: { patient_id: patientId }, skip: !patientId || !historyOpen })
  ...
  const applyFetchedItems = (items) => {
    setLines(items.map((i) => ({
      ...
      dose: i.dose, frequency: i.frequency, route: i.route ?? '',
      duration_days: i.duration_days ?? '', qty: i.qty ?? '',
      instructions: i.instructions ?? '', substitutable: i.substitutable ?? true,
    })))
  }
  ...
  const validLines = useMemo(
    () => lines.filter((l) => l.drug && l.dose && l.frequency),
    [lines],
  )

  const buildItemsInput = () => validLines.map((l) => ({
    drug_id: l.drug.id, dose: l.dose, frequency: l.frequency,
    route: l.route || undefined, duration_days: l.duration_days ? Number(l.duration_days) : undefined,
    instructions: l.instructions || undefined, substitutable: l.substitutable,
  }))
  ...
  return (
    <Box p={{ xs: 1.5, md: 3 }}>
      <Stack direction="row" spacing={1.5} alignItems="center" mb={2}>
        ...
        <Typography variant="h6" fontWeight={700}>New Prescription</Typography>
      </Stack>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <Paper variant="outlined" sx={{ p: 2 }}>
              ...
              <TableBody>
                {lines.map((line, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      <TextField size="small" value={line.dose} onChange={(e) => updateLine(idx, { dose: e.target.value })} placeholder="500mg" />
                    </TableCell>
                    <TableCell>
                      <FormControl size="small" sx={{ minWidth: 90 }}>
                        <Select inputProps={{ 'aria-label': 'Frequency' }} value={line.frequency} onChange={(e) => updateLine(idx, { frequency: e.target.value })}>
                          {FREQUENCIES.map((f) => <MenuItem key={f} value={f}>{f}</MenuItem>)}
                        </Select>
                      </FormControl>
                    </TableCell>
                    <TableCell>
                      <TextField size="small" value={line.route} onChange={(e) => updateLine(idx, { route: e.target.value })} placeholder="Oral" />
                    </TableCell>
                    <TableCell>
                      <TextField size="small" type="number" value={line.duration_days} onChange={(e) => updateLine(idx, { duration_days: e.target.value })} sx={{ width: 90 }} />
                    </TableCell>
                    <TableCell>
                      <TextField size="small" type="number" value={line.qty} onChange={(e) => updateLine(idx, { qty: e.target.value })} sx={{ width: 70 }} />
                    </TableCell>
                    <TableCell>
                      <TextField size="small" value={line.instructions} onChange={(e) => updateLine(idx, { instructions: e.target.value })} placeholder="After food" />
                    </TableCell>
                    <TableCell>
            ...
            <Button startIcon={<AddRoundedIcon />} onClick={addLine} sx={{ mt: 1.5 }}>Add Line</Button>

            <Divider sx={{ my: 2 }} />

            <Stack direction="row" spacing={1.5}>
              <Button
                variant="contained" onClick={handleIssue} disabled={issuing || validLines.length === 0}
              >
                Issue Prescription
              </Button>
        ...
        <Grid item xs={12} md={4}>
          <Paper variant="outlined" sx={{ p: 2 }}>
            <Typography variant="subtitle2" fontWeight={700} mb={1}>Favourite Sets</Typography>
            <List dense disablePadding>
              {sets.length === 0 && <Typography variant="body2" color="text.secondary">No favourite sets yet.</Typography>}
              {sets.map((s) => (
        ...
        <DialogContent>
          <List dense>
            ...
            {(historyData?.patientPrescriptions ?? []).length === 0 && (
              <Typography variant="body2" color="text.secondary" sx={{ p: 2 }}>No prior prescriptions.</Typography>
            )}
          </List>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={saveSetOpen} onClose={() => setSaveSetOpen(false)}>
        ...
        <DialogContent>
          <TextField
            fullWidth sx={{ mt: 1 }} label="Set name" value={setName} onChange={(e) => setSetName(e.target.value)}
            placeholder="URI adult set"
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSaveSetOpen(false)}>Cancel</Button>
          <Button variant="contained" onClick={handleSaveSet} disabled={!setName.trim()}>Save</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
