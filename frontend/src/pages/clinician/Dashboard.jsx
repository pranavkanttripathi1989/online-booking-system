...
function assignOverlapColumns(appts) {
  ...
  return sorted
    .map((appt) => {
      ...
      if (!placed) {
        columns.push([appt])
        return { ...appt, _col: columns.length - 1 }
      }
      return { ...appt, _col: 0 }
    })
...
export default function ClinicianDashboard() {
  ...
  const {
    ...
    error: profileError,
  } = useQuery(GET_MY_CLINICIAN_PROFILE, {
  ...
  if (!profileLoading && profileData && !profileData.me?.clinician) {
    return (
      <Box p={{ xs: 2, md: 4 }} maxWidth="xl" mx="auto">
        <Alert severity="warning">Your account isn't linked to a clinician profile yet — contact your admin.</Alert>
      </Box>
    )
  }
  ...
  const displayName = profileData?.me?.clinician?.full_name ? `Dr. ${profileData.me.clinician.full_name}` : 'Dr. —'
  ...
  const handleSaveBlock = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Block ${dayjs(`2000-01-01T${blockForm.startTime}`).format('h:mm A')}–${dayjs(`2000-01-01T${blockForm.endTime}`).format('h:mm A')} added to schedule.`,
        severity: 'success',
      })
    } catch (e) {
            ...
            <Box ref={timelineRef} position="relative" height={TIMELINE_HEIGHT} overflow="auto" sx={{ bgcolor: 'background.default' }}>
              ...
              {apptWithCols.map((appt) => {
                ...
                return (
                  <Tooltip
                    key={appt.id}
                    title={`${appt.patient.full_name} · ${dayjs(`${todayStr}T${appt.startTime}`).format('h:mm A')}`}
                    placement="left"
                  >
                    <Card
                      elevation={0}
                      ...
                      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                        ...
                        <Stack alignItems="flex-end" flexShrink={0}>
                          <Typography variant="caption" fontWeight={700} sx={{ fontSize: '0.65rem' }}>
                            {dayjs(`${todayStr}T${appt.startTime}`).format('h:mm A')}
                          </Typography>
                  ...
                  <Stack direction="row" gap={2} alignItems="center" mb={2}>
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" color="text.secondary" fontWeight={500}>
                        {dayjs(`${todayStr}T${nextAppt.startTime}`).format('h:mm A')} · {nextAppt.duration || 30} mins ·{' '}
                        {nextAppt.type === 'video' ? 'Video' : 'In-Person'}
                      </Typography>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Typography variant="caption" color="text.secondary" sx={{ fontFamily: 'monospace' }}>
                            {dayjs(`${todayStr}T${appt.startTime}`).format('h:mm A')} · {appt.product?.name}
                          </Typography>
                        }
                      />
          ...
          <>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
              <Stack direction="row" gap={2} alignItems="center" mb={3}>
                ...
                >
                  {patientInitials(selectedAppt.patient.full_name)}
                </Avatar>
                <Box>
                  <Typography variant="subtitle1" fontWeight={800}>
            ...
            <Box sx={{ p: 3, borderTop: '1px solid', borderTopColor: 'divider', flexShrink: 0 }}>
              <Stack spacing={1.5}>
                ...
                <Button
                  variant="outlined"
                  fullWidth
                  onClick={() => navigate(`/patients/${selectedAppt.patient.id}`)}
