...
import {
  Box, Typography, Grid, Paper, Avatar, Chip, Stack, Button,
  Card, CardContent, ToggleButtonGroup, ToggleButton, CircularProgress, Alert, Divider, Skeleton
} from '@mui/material';
import {
  CalendarMonth, Videocam, LocationOn, VerifiedUser, LocalHospital, School, MedicalServices
...
export default function DoctorProfile() {
  ...
  if (!data?.getClinician) return <Box p={4}><Alert severity="warning">Clinician not found</Alert></Box>;
        ...
        <Grid container spacing={3} alignItems="center">
          ...
          <Grid item xs={12} md={6}>
            ...
            <Stack direction="row" gap={1} mt={1.25} alignItems="center">
              ...
              <Typography variant="body2" color="text.secondary">{clinician.clinic?.address || clinician.clinic?.name || 'Location'}</Typography>
            </Stack>
            <Stack direction="row" gap={1} mt={0.5} alignItems="center">
              <VerifiedUser fontSize="small" sx={{ color: '#10B981' }} />
              <Typography variant="body2" sx={{ color: '#10B981', fontWeight: 600 }}>Verified Practitioner</Typography>
            </Stack>
            <Divider sx={{ my: 1.5 }} />
            <Stack direction="row" flexWrap="wrap" gap={0.75}>
              {clinician.languages?.map((lang, index) => (
                <Chip key={index} label={lang.name} size="small" sx={{ bgcolor: '#F1F5F9', color: 'text.secondary', fontWeight: 600, fontSize: '0.7rem', height: 22, borderRadius: '6px' }} />
              ))}
            </Stack>
          </Grid>
          <Grid item xs={12} md="auto" sx={{ ml: 'auto', textAlign: { xs: 'left', md: 'right' } }}>
            <Button
              ...
              sx={{ bgcolor: BRAND, '&:hover': { bgcolor: '#005B64' }, borderRadius: 2, fontWeight: 700, mb: 1.5, display: 'block', width: '100%' }}
            >
              Book Appointment
            </Button>
            <Button
              ...
              size="large"
              startIcon={<Videocam />}
              sx={{ borderColor: BRAND, color: BRAND, '&:hover': { bgcolor: `${BRAND}08` }, borderRadius: 2, fontWeight: 600, width: '100%' }}
            >
              Video Consultation
            </Button>
          </Grid>
          ...
          <Card elevation={0} sx={{ border: '1px solid #E2E8F0', borderRadius: 3, mb: 3 }}>
            <CardContent sx={{ p: 3 }}>
              <Stack direction="row" alignItems="center" gap={1} mb={1.5}>
                ...
                <Typography variant="overline" fontWeight={800} color={BRAND} letterSpacing={1}>About</Typography>
              </Stack>
              <Typography variant="body1" color="text.secondary" lineHeight={1.8}>{clinician.bio || 'No bio available.'}</Typography>
            </CardContent>
          </Card>
          ...
          <Card elevation={0} sx={{ border: '1px solid #E2E8F0', borderRadius: 3, mb: 3 }}>
            <CardContent sx={{ p: 3 }}>
              <Stack direction="row" alignItems="center" gap={1} mb={2}>
                ...
                <Typography variant="overline" fontWeight={800} color={BRAND} letterSpacing={1}>Services</Typography>
              </Stack>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {clinician.products?.map(product => (
                  <Chip
                    ...
                    label={product.name}
                    size="small"
                    sx={{ bgcolor: `${BRAND}10`, color: BRAND, fontWeight: 600, fontSize: '0.75rem', height: 26, borderRadius: 1.5, border: `1px solid ${BRAND}30` }}
                  />
                ))}
              </Box>
            </CardContent>
          ...
          <Card elevation={0} sx={{ border: '1px solid #E2E8F0', borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              <Stack direction="row" alignItems="center" gap={1} mb={2}>
                ...
                <Typography variant="overline" fontWeight={800} color={BRAND} letterSpacing={1}>Education & Experience</Typography>
              </Stack>
            ...
            <Paper elevation={0} sx={{ p: 3, border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
              ...
              <Box mt={2} id="slot-container">
                {availableSlots.length > 0 ? (
                  ...
                  <Typography variant="body2" color="text.secondary" textAlign="center" py={2}>No slots available on this date.</Typography>
                )}
              </Box>
