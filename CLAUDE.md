...
`backend/src` has 22 built domain modules; **all 22** (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `services`, `staff`, `test-results`, `users`) now have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`) — the unit-test half of this priority's DoD is done. The remaining gap is e2e: `frontend/e2e/` now has real-backend specs for 21 domains (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `services`, `test-results`, `users`) — each one confirmed real (not mock-fallback) via a Chrome MCP live check before the spec was written (`context/qa-full-inventory.md` §7), not assumed; four of those (`analytics`, `public`, `services`, `staff` — see below) required finding the *right* page to target or fixing a real bug first, since the obvious route was either mock-only or broken. `admin-roles.spec.js` (pre-existing) doesn't count toward this — it exercises `admin/Roles.jsx`, which is still 100% `mocks/store.js`-driven. `public` needed two real fixes before a spec was possible (`@Public()` on `getClinicianAvailability`, `App.jsx`'s `OptionalAuthShell`) — `pages/public/landing.jsx` itself is still mock, so its specs go straight to `/doctor/:id`/`/appointments/book` with a real clinician id instead. `services` needed a full rewrite of `manager/services/index.jsx` against the real `services`/`productCategories` contract (its Variations/Cancellation-Rules tabs were removed entirely — grepped the backend and confirmed neither has ever had a resolver, not a wiring gap) plus a real backend bug fix (`ServicesService.toGraphQL()` was passing the raw Prisma `Clinicians` row through instead of computing `full_name`, crashing the whole query for any service with a linked clinician — the existing unit test had asserted on the buggy shape and missed it; fixed both the bug and the test). Only `staff` remains without an e2e spec — confirmed via the network inspector its frontend is still 100% mock (zero GraphQL requests fired), matching the already-known gap; blocked on that frontend wiring, not on missing test-writing effort. Full e2e suite last confirmed fully green at 26/26 before the `services` fix; the `services` addition (27th spec) passed in isolation but a fresh full-suite re-run couldn't complete afterward due to severe host resource contention on the dev machine (not a code issue — confirmed via a direct bare Playwright browser-launch timing test) — re-run `npm run e2e` (`--workers=1`) once the machine has headroom to get a final full-suite confirmation.
...
**DoD:** every existing domain module has unit test coverage for happy path + tenant isolation + self-scoping + role gating (✅ done, all 22), and at least one e2e path is green against the real backend, not mocks (🟡 21 of 22 domains have a real-backend e2e spec; only `staff` remains, blocked on its own logged frontend gap — full-suite green last confirmed at 26/26, the 27th `services` spec passed in isolation but needs a fresh full-suite re-run once the dev machine isn't under heavy load).
