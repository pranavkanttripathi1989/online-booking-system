...
npm test                  # jest — 8 of 22 domains covered so far (see Current priorities for which)
...
`backend/src` has 22 built domain modules; 8 (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `patients`, `test-results`) have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`). The remaining 14 (`clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `products`, `public`, `reviews`, `rooms`, `services`, `staff`, `users`) are still the gap — every subsequent step compounds on untested code.

1. Pick one domain at a time from the untested-14 list above.
