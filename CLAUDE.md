...
**DoD:** every existing domain module has unit test coverage for happy path + tenant isolation + self-scoping + role gating (✅ done, all 22), and at least one e2e path is green against the real backend, not mocks (🟡 21 of 22 domains have a real-backend e2e spec, full suite confirmed green at 27/27; only `staff` remains, blocked on its own logged frontend gap — a genuine, not test-writing, gap).
