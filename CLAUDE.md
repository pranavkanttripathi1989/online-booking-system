...
npm test                  # jest — 16 of 22 domains covered so far (see Current priorities for which)
...
`backend/src` has 22 built domain modules; 16 (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `organizations`, `patients`, `public`, `staff`, `test-results`) have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`). The remaining 6 (`notifications`, `products`, `reviews`, `rooms`, `services`, `users`) are still the gap — every subsequent step compounds on untested code.
