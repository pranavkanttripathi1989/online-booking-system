...
`backend/src` has 22 built domain modules; **all 22** (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `services`, `staff`, `test-results`, `users`) now have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`) — the unit-test half of this priority's DoD is done. The remaining gap is e2e: `frontend/e2e/` now has real-backend specs for 20 domains (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `test-results`, `users`) — each one confirmed real (not mock-fallback) via a Chrome MCP live check before the spec was written (`context/qa-full-inventory.md` §7), not assumed; three of those (`analytics`, `public`, `staff` — see below) required finding the *right* page to target or fixing a real bug first, since the obvious route was either mock-only or broken. Full e2e suite (26 tests) confirmed green (`npm run e2e` in `frontend/`); run serialized (`--workers=1`) if the dev machine is memory-constrained — parallel workers' concurrent logins were observed to intermittently race the backend/frontend dev server under load. `admin-roles.spec.js` (pre-existing) doesn't count toward this — it exercises `admin/Roles.jsx`, which is still 100% `mocks/store.js`-driven. `public` needed two real fixes before a spec was possible (both shipped, see availability.resolver.ts's `@Public()` on `getClinicianAvailability` and App.jsx's `OptionalAuthShell`) — `pages/public/landing.jsx` itself is still mock (own `MOCK_DOCTORS` array), so its specs go straight to `/doctor/:id` and `/appointments/book` with a real clinician id instead. Only 2 domains remain without an e2e spec: `services` (its frontend page's query doesn't match either GraphQL dialect and 400s on every load — real bug, logged in `context/open-questions.md`/`qa-full-inventory.md` §7, not fixed) and `staff` (confirmed via the network inspector its frontend is still 100% mock, zero GraphQL requests fired — matches the already-known gap). Both are blocked on their own logged frontend bugs, not on missing test-writing effort — treat Priority 1's e2e sub-goal as functionally done until either gets fixed.
...
**DoD:** every existing domain module has unit test coverage for happy path + tenant isolation + self-scoping + role gating (✅ done, all 22), and at least one e2e path is green against the real backend, not mocks (🟡 20 of 22 domains have a real-backend e2e spec; `services`/`staff` are blocked on their own logged frontend gaps, not on missing test-writing).
