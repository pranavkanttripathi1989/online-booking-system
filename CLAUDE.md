...
`backend/src` has 22 built domain modules; **all 22** (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `services`, `staff`, `test-results`, `users`) now have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`) — the unit-test half of this priority's DoD is done. The remaining gap is e2e: `frontend/e2e/` now has real-backend specs for 19 domains (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `reviews`, `rooms`, `test-results`, `users`) — each one confirmed real (not mock-fallback) via a Chrome MCP live check before the spec was written (`context/qa-full-inventory.md` §7), not assumed; two of those (`analytics`, `staff` — see below) required finding the *right* page to target, since the obvious route was mock-only. Full e2e suite (24 tests) confirmed green (`npm run e2e` in `frontend/`); run serialized (`--workers=1`) if the dev machine is memory-constrained — parallel workers' concurrent logins were observed to intermittently race the backend/frontend dev server under load. `admin-roles.spec.js` (pre-existing) doesn't count toward this — it exercises `admin/Roles.jsx`, which is still 100% `mocks/store.js`-driven. Still missing an e2e spec: `public` (patient self-serve booking wizard — needs a dedicated pass, not yet done) and `services` (its frontend page's query doesn't match either GraphQL dialect and 400s on every load — real bug, logged in `context/open-questions.md`/`qa-full-inventory.md` §7, not fixed). `staff` also has no spec — confirmed via the network inspector its frontend is still 100% mock (zero GraphQL requests fired), matching the already-known gap. Don't treat this priority as closed until `public` gets a spec (the `services` gap is blocked on the logged bug, not on test-writing).
...
**DoD:** every existing domain module has unit test coverage for happy path + tenant isolation + self-scoping + role gating (✅ done, all 22), and at least one e2e path is green against the real backend, not mocks (🟡 in progress — 19 of 22 domains have a real-backend e2e spec today; `public` still needs one, `services`/`staff` are blocked on their own logged frontend gaps).
