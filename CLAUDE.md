...
`backend/src` has 22 built domain modules; **all 22** (`auth`, `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `notifications`, `organizations`, `patients`, `products`, `public`, `reviews`, `rooms`, `services`, `staff`, `test-results`, `users`) now have `.spec.ts` coverage, plus both global guards (`common/guards/*.spec.ts`) — the unit-test half of this priority's DoD is done. The remaining gap is e2e: `frontend/e2e/` now has real-backend specs for 10 domains (`auth`, `clinics`, `email-templates`, `languages`, `lookups`, `messages`, `products`, `reviews`, `rooms`, `users`) — each one confirmed real (not mock-fallback) via a Chrome MCP live check before the spec was written (`context/qa-full-inventory.md` §7), not assumed. Full e2e suite (15 tests) confirmed green (`npm run e2e` in `frontend/`); run serialized (`--workers=1`) if the dev machine is memory-constrained — parallel workers' concurrent logins were observed to intermittently race the backend/frontend dev server under load. `admin-roles.spec.js` (pre-existing) doesn't count toward this — it exercises `admin/Roles.jsx`, which is still 100% `mocks/store.js`-driven. Still missing an e2e spec: `analytics`, `appointments`, `availability`, `blocks`, `clinicians`, `notifications`, `organizations`, `patients`, `public`, `services`, `staff`, `test-results` — `services` specifically can't get one yet since its frontend page's query doesn't match either GraphQL dialect and 400s on every load (real bug, logged in `context/open-questions.md`/`qa-full-inventory.md` §7, not fixed). Don't treat this priority as closed until the remaining 12 are addressed.
...
**DoD:** every existing domain module has unit test coverage for happy path + tenant isolation + self-scoping + role gating (✅ done, all 22), and at least one e2e path is green against the real backend, not mocks (🟡 in progress — 10 of 22 domains have a real-backend e2e spec today).
