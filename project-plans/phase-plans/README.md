...
current_slice:  P2-17            # next unstarted, unblocked slice — P2-01 depends on blocked P1-10; P2-02..P2-16 done (P2-07 shipped allergy-only; P2-08 shipped English/Hindi only; P2-09 shipped all 6 offered languages, not just 3, per explicit user selection, see REQ161; P2-10 shipped as a new AppointmentSeries domain, see REQ163; P2-11 shipped as a new ImmunizationScheduleItems/ImmunizationRecords domain, see REQ167; P2-12 shipped as a new ChronicRegistryEnrollments domain, see REQ168; P2-13 shipped as recordTestResult -- the phase tracker's own "results inbox" framing was itself superseded by a more fundamental missing-mutation finding, see REQ184; P2-14 shipped as a backend-only encounter-seeding change with zero frontend surface required, see REQ185; P2-15 shipped as a frontend-only kiosk UI reusing REQ107's existing checkInWithQrToken mutation verbatim, see REQ186; P2-16 shipped a new reschedule-token flow plus a real pre-existing reminder_count reset bug fix, see REQ187)
last_completed: REQ187           # P2-16 (self-serve reschedule link in every reminder -- AppointmentReminderSweepService mints a single-use reschedule token and includes a real /reschedule/:token link in the SMS/WhatsApp text) closed 2026-09-03.
