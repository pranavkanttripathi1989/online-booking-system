...
| **P3-10** | ~~IPD-lite (beds, admissions, discharge summaries)~~ — superseded 2026-09-02 | BE+FE | **done**, out of sequence | — | A direct customer decision widened this to the FULL system (nursing home/hospital/ICU/ward/OT/billing), overriding the "not what a hospital information system would want" framing this row and its Phase-3 placement assumed. All 5 slices shipped 2026-09-02, ahead of this row's own P3 sequencing: ADT core/wards/beds/MLC (`REQ179`), nursing charting/MAR/discharge summary (`REQ180`), OT scheduling (`REQ181`), billing ledger/room-day accrual/package settlement (`REQ182`), TPA cashless insurance (`REQ183`) — see `requirements/ipd/README.md`. Deliberately still out of scope even under the widened brief: real payer/NHCX API integration, government-scheme claims (PMJAY/CGHS), `IpdClaimAppeals` — see `REQ183`'s own account. |
