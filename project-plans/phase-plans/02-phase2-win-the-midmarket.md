...
| **P2-15** | Kiosk check-in mode | FE only (reuses `checkInWithQrToken` verbatim — see `REQ186`) | **done** (`REQ186`) | — | QR flow existed (`REQ107`) but only for a patient's own phone. Shipped: a new bare `/checkin` route, mounted outside `PublicLayout` (no marketing nav/login link on a shared kiosk device), with an idle "Scan your QR code" screen driven by an always-focused hidden input that captures a standard keyboard-wedge scanner's keystrokes — no camera/QR-decode library added. Extracts the token from either the full scanned URL or a bare token, calls the same mutation, and auto-resets to idle 6s after any result so the next patient never sees the previous one's outcome. `/checkin/:token` (personal-phone flow) unchanged. See `REQ186`'s own account |
| **P2-16** | Self-serve reschedule link in every reminder | BE+FE | **done** (`REQ187`) | P1-01 | Deflects front-desk calls cheaply. Shipped: `AppointmentReminderSweepService` now mints a single-use, time-boxed reschedule token (same hash-only shape as `REQ107`'s `checkin_token`) and includes a real `/reschedule/:token` link in the SMS/WhatsApp text — previously the only link anywhere was in-app-only, never reaching the external channel. New public `getRescheduleContext`/`reschedulePublicAppointment`, reusing `assertSlotFree` and a newly-extracted reschedule-fee helper (`REQ177`). A real pre-existing bug fixed along the way: neither `update()` nor anything else ever reset `reminder_count`/`reminder_sent_at` on a reschedule, so a rescheduled appointment could silently never get another reminder for its new time. See `REQ187`'s own account |
