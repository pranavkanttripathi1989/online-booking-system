...
| **P2-13** | ~~Investigation orders +~~ results inbox (lab loop) | BE+FE | not started | — | `REQ020` P1. **Scope corrected 2026-08-30** (`project-plans/analysis/14-mocdoc-competitive-analysis-and-roadmap.md`) — "investigation orders" already shipped via `REQ127`'s real `orderInvestigation()`; the remaining, genuinely unbuilt half is a results-inbox worklist (pending/incoming results across a clinician's/lab's own patients), which the current all-results `test-results/index.jsx` list page isn't |
