...
**2026-08-19 — Priority 1 unit coverage added for what actually exists today** (`backend/src/organizations/organizations.service.spec.ts`, `organizations.resolver.spec.ts`, 32 tests, all green): covers `normalizeOrgCode` (TC-ONBOARD-UNIT-001/008's normalization half — the "reject a duplicate" half is covered via `create`/`update` conflict tests), `create`'s ownerless/`onboarding_status: completed` default (TC-ONBOARD-UNIT-009), code-uniqueness rejection on both `create` and `update` (ConflictException, no write attempted), `update`/`softDelete`'s `NotFoundException` on a missing or already-soft-deleted org, pagination math, and the resolver's `toResult()` HttpException→`{success,userErrors}` mapping (including the multi-message `BadRequestException` case and re-throw of non-`HttpException` errors) plus a role-gating check that all four handlers are `@Auth('admin','super_admin')` only. This closes CLAUDE.md's Priority-1 gap for the `organizations` backend module specifically — it does **not** touch TC-ONBOARD-UNIT-002 through 007/010-012 or any API-/E2E-tier case, all of which require the not-yet-built onboarding wizard, entitlements guard, or branding backend and remain exactly as forward-looking as described above.
