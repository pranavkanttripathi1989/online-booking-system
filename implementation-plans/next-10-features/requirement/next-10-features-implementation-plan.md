...
Not yet done: unit/integration tests per `backend-hard-rules.md` Rule 7 (curl + live-data verification only, no `.spec.ts` files written this pass — same gap flagged for every prior increment), a live Playwright-MCP browser pass against the actual frontend pages (curl-verified against the schema/resolvers directly, not yet clicked through in a browser), and Reviews' `avg_rating`/`total_reviews` computed fields on `Clinician` (flagged as additive/optional in the plan, deferred).

## Phase P8 (public/patient-self-serve booking) — also built, migrated, live-verified (2026-08-18)

`backend/src/public/**` — `getClinicians`/`getClinician`/`getProducts`/`getAppointments`/`getAppointment`, `bookPatientAppointment`, `createPaymentTransaction`, all `@Public()` except `getAppointment` (video/index.jsx, behind login). Resolved the one real GraphQL-level conflict this phase always implied: `booking/index.jsx`'s camelCase `createAppointment`/`AppointmentInput` collided by name with the already-live canonical snake_case `createAppointment`/`AppointmentInput` — GraphQL cannot register two resolvers or two input types under the same name. Renamed the public-facing one to `bookPatientAppointment`/`BookPatientAppointmentInput` (that page had zero backend before this pass, so nothing broke) and updated its own inline `gql` accordingly; also fixed its currency display/submission from a stray `£`/`GBP` to `₹`/`INR` while in the file, per CLAUDE.md's India-market decision. `getClinician` (shared by `clinician/Availability.jsx`, `booking/index.jsx`, `doctor-profile.jsx`) moved fully into this module — it needed one comprehensive type serving all three callers' differing selections, which can't be split across resolvers.

Scope cuts, documented not silent: `education` on `PublicClinician` always returns `[]` (no backing model exists); `nextAvailable` on `getClinicians` search results is omitted (would mean running the slot algorithm per clinician per search request); `createPaymentTransaction` records the transaction row only — no real Stripe/Razorpay API call (no gateway secrets configured in this environment). `public/landing.jsx` itself is still not wired to `getClinicians` — it remains a `setTimeout`-simulated mock; the real query it needs now exists and is verified, wiring the page is a small remaining frontend task, not a backend gap.
