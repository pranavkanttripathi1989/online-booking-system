...
describe('NotificationsResolver', () => {
  ...
  let service: { findAll: jest.Mock; markRead: jest.Mock; markAllRead: jest.Mock; remove: jest.Mock };
  const reflector = new Reflector();

  beforeEach(async () => {
    service = { findAll: jest.fn(), markRead: jest.fn(), markAllRead: jest.fn(), remove: jest.fn() };
  ...
  describe('argument passthrough', () => {
    it('notifications forwards filter and user', async () => {
      const user = { sub: 'user-1' } as any;
      service.findAll.mockResolvedValue([]);
      await resolver.notifications('unread', user);
      expect(service.findAll).toHaveBeenCalledWith('unread', user);
    });
