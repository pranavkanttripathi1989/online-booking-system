import { Controller, Post, Req, UploadedFile, UseInterceptors, UnauthorizedException, BadRequestException } from '@nestjs/common';
...
const UPLOAD_DIR = path.join(__dirname, '..', '..', 'uploads', 'ipd-insurance-documents');

// REQ179 (IPD slice 5) -- plain REST endpoint mirroring
// encounters/attachments.controller.ts's / messages/message-attachments
// .controller.ts's own pattern exactly (multer in-memory, magic-byte
// signature allow-list, manual bearer verify since the global
// GqlAuthGuard only protects the GraphQL execution context). No role
// check here, matching message-attachments.controller.ts's own stance --
// the real access check lives in the GraphQL createIpdInsuranceDocument
// mutation that persists the DB row (FRONT_DESK_ROLES+), the same
// two-step upload-then-persist split.
...
@Controller('ipd-insurance-documents')
export class IpdInsuranceAttachmentsController {
  ...
  async upload(@UploadedFile() file: Express.Multer.File, @Req() req: Request) {
    ...
    return { file_ref: `/uploads/ipd-insurance-documents/${filename}`, mime_type: signature.mime };
  }
}
