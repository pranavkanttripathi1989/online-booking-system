...
export class MarService {
  ...
  async administer(input: AdministerMedicationInput, user: JwtPayload) {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (input.status === 'given' && input.batch_id) {
        stockMovementId = await this.consumeStock(tx, input.batch_id, mar.drug_id, mar.client_org_id, mar.id, user.sub);
      }
      return tx.medicationAdministrations.update({
        where: { id: input.mar_id },
        data: {
          status: input.status,
  ...
  async recordPrn(input: RecordPrnAdministrationInput, user: JwtPayload) {
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      let stockMovementId: string | undefined;
      if (input.status === 'given' && input.batch_id) {
        stockMovementId = await this.consumeStock(tx, input.batch_id, order.drug_id, order.client_org_id, mar.id, user.sub);
      }
      if (stockMovementId) {
        return tx.medicationAdministrations.update({
          where: { id: mar.id },
      ...
      return tx.medicationAdministrations.findUnique({ where: { id: mar.id }, include: this.MAR_INCLUDE });
    });
